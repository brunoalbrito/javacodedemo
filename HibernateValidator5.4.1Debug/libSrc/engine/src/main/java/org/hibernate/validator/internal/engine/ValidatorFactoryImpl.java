...
public class ValidatorFactoryImpl implements HibernateValidatorFactory {
	...
	public ValidatorFactoryImpl(ConfigurationState configurationState) {
		// configurationState === org.hibernate.validator.internal.engine.ConfigurationImpl
		...
		if ( configurationState instanceof ConfigurationImpl ) {
			...
			// hibernateSpecificConfig.getMethodValidationConfiguration() === org.hibernate.validator.internal.engine.MethodValidationConfiguration
		...
		// constraintHelper === org.hibernate.validator.internal.metadata.core.ConstraintHelper
		registerCustomConstraintValidators( constraintMappings, constraintHelper ); // !!! 注解和相应的处理器
		...
		tmpFailFast = checkPropertiesForBoolean( properties, HibernateValidatorConfiguration.FAIL_FAST, tmpFailFast ); // 读取属性文件获取配置
		this.failFast = tmpFailFast;

		this.methodValidationConfiguration = new MethodValidationConfiguration();

		tmpAllowOverridingMethodAlterParameterConstraint = checkPropertiesForBoolean( // 读取属性文件获取配置
				properties,
	...
	private static Set<DefaultConstraintMapping> getConstraintMappings(ConfigurationState configurationState, ClassLoader externalClassLoader) {
		...
		if ( configurationState instanceof ConfigurationImpl ) { // !!!!
			...
			// !!! serviceLoaderBasedContributor === org.hibernate.validator.internal.engine.ServiceLoaderBasedConstraintMappingContributor
			...
			serviceLoaderBasedContributor.createConstraintMappings( builder ); // !!!! 扫描自定义校验器注入constraintMappings
		}
	...
	public Validator getValidator() {
		// constraintValidatorManager === org.hibernate.validator.internal.engine.constraintvalidation.ConstraintValidatorManager
		return createValidator(
				...
				failFast, // 快速失败
				validatedValueHandlers, // { org.hibernate.validator.internal.engine.valuehandling.OptionalValueUnwrapper }
				timeProvider,  // org.hibernate.validator.internal.engine.time.DefaultTimeProvider
				methodValidationConfiguration // org.hibernate.validator.internal.engine.MethodValidationConfiguration
		);
	}
	...
	private static void registerCustomConstraintValidators(Set<DefaultConstraintMapping> constraintMappings,
		...
		for ( DefaultConstraintMapping constraintMapping : constraintMappings ) {
			...
			for ( ConstraintDefinitionContribution<?> contribution : constraintMapping.getConstraintDefinitionContributions() ) {
				// contribution === org.hibernate.validator.internal.engine.constraintdefinition.ConstraintDefinitionContribution
	...
	private static <A extends Annotation> void processConstraintDefinitionContribution(
		...
		// constraintType === cn.java.demo.constraintsx.Money自定义注解
