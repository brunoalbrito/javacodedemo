...
public abstract class TypeResolverBasedValueUnwrapper<T> extends ValidatedValueUnwrapper<T> {
	...
	TypeResolverBasedValueUnwrapper(TypeResolutionHelper typeResolutionHelper) {
		// typeResolutionHelper === org.hibernate.validator.internal.util.TypeResolutionHelper
		this.typeResolver = typeResolutionHelper.getTypeResolver(); // typeResolver === com.fasterxml.classmate.TypeResolver
		clazz = resolveSingleTypeParameter( typeResolver, this.getClass(), ValidatedValueUnwrapper.class ); // class == java.util.Optional
	}
	...
	private static Class<?> resolveSingleTypeParameter(TypeResolver typeResolver, Type subType, Class<?> target) {
		// subType === org.hibernate.validator.internal.engine.valuehandling.OptionalValueUnwrapper
