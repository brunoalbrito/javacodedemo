...
public class JdbcServicesImpl implements JdbcServices, ServiceRegistryAwareService, Configurable {
	...
	public void injectServices(ServiceRegistryImplementor serviceRegistry) {
		this.serviceRegistry = serviceRegistry; // org.hibernate.boot.registry.internal.StandardServiceRegistryImpl
	}

	@Override
	public void configure(Map configValues) {
		//!!! ConnectionProviderJdbcConnectionAccess 创建数据库连接池
		// jdbcConnectionAccess === org.hibernate.engine.jdbc.internal.JdbcServicesImpl.ConnectionProviderJdbcConnectionAccess
		final JdbcConnectionAccess jdbcConnectionAccess = buildJdbcConnectionAccess( configValues );
		
		/*
		  dialectFactory === org.hibernate.engine.jdbc.dialect.internal.DialectFactoryInitiator.initiateService(...)
		  dialectFactory === org.hibernate.engine.jdbc.dialect.internal.DialectFactoryImpl
		 */
		...
		if ( useJdbcMetadata ) {//true 
			try {
				//数据库连接
				// jdbcConnectionAccess === org.hibernate.engine.jdbc.internal.JdbcServicesImpl.ConnectionProviderJdbcConnectionAccess
				// connection === org.hibernate.engine.jdbc.internal.JdbcServicesImpl.ConnectionProviderJdbcConnectionAccess.obtainConnection();
				// connection === org.hibernate.engine.jdbc.connections.internal.DriverManagerConnectionProviderImpl.getConnection();
				final Connection connection = jdbcConnectionAccess.obtainConnection(); // 数据库连接
				try {
...
//						schemaNameResolver = dialect.getSchemaNameResolver();
		...
		this.extractedMetaDataSupport = new ExtractedDatabaseMetaDataImpl(
				...
				parseKeywords( extraKeywordsString ),//数据库的关键字
	...
	private static class ConnectionProviderJdbcConnectionAccess implements JdbcConnectionAccess {
		...
		public ConnectionProviderJdbcConnectionAccess(ConnectionProvider connectionProvider) {
			this.connectionProvider = connectionProvider; // DriverManagerConnectionProviderImpl
		}
	...
	private SchemaNameResolver determineExplicitSchemaNameResolver(Map configValues) {
		final Object setting = configValues.get( SCHEMA_NAME_RESOLVER ); // hibernate.schema_name_resolver
