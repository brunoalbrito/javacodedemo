...
public class ThreadLocalSessionContext extends AbstractCurrentSessionContext {
	...
	public final Session currentSession() throws HibernateException {
		//existingSession( SessionFactoryImpl);
		Session current = existingSession( factory() );
		if ( current == null ) {
			/**
			 
			 */
			current = buildOrObtainSession();//current == new org.hibernate.internal.SessionImpl
			
			/**
				 SessionImpl()
				 {
				 	SessionImpl.transactionCoordinator = new TransactionCoordinatorImpl( connection, this );
				 	SessionImpl.transactionCoordinator.getJdbcCoordinator().getLogicalConnection().addObserver(
						new ConnectionObserverStatsBridge( factory )
				 }
				 
				 SessionImpl.getTransaction().registerSynchronization( buildCleanupSynch() );
				 {
				 	TransactionCoordinatorImpl.getTransaction().registerSynchronization( buildCleanupSynch() );
				 	{
				 		currentHibernateTransaction.registerSynchronization( buildCleanupSynch() );
				 	}
				 }
	...
	protected Session buildOrObtainSession() {
		/**
		 	//org.hibernate.internal.SessionFactoryImpl.SessionBuilderImpl 内部类

		 	sessionBuilderImpl = new SessionBuilderImpl( SessionFactoryImpl );
		 	sessionBuilderImpl.autoClose( true )
		 	sessionBuilderImpl.connectionReleaseMode( SessionFactoryImpl.getSettings().getConnectionReleaseMode(); )
		 	sessionBuilderImpl.flushBeforeCompletion( true )
		 	sessionBuilderImpl.openSession();
		 */
		return baseSessionBuilder()//SessionBuilderImpl
