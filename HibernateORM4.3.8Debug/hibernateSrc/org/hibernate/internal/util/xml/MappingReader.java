...
public class MappingReader {
	...
	/**
	  	...
	  	readMappingDocument(new EJB3DTDEntityResolver(), 解析出student.hbm.xml映射文件输入流, new OriginImpl( "resource", "cn/java/hibernate/entity/student.hbm.xml" ) )
	 */
...
//		return readMappingDocument( source, origin );
	...
	/**
	 	legacyReadMappingDocument(new EJB3DTDEntityResolver(), 解析出student.hbm.xml映射文件输入流, new OriginImpl( "resource", "cn/java/hibernate/entity/student.hbm.xml" ) )
	 */
	private XmlDocument legacyReadMappingDocument(EntityResolver entityResolver, InputSource source, Origin origin) {
		...
		try {
			...
			document = saxReader.read( source );//解析cn/java/hibernate/entity/student.hbm.xml文件
			if ( errorHandler.hasErrors() ) {
				throw errorHandler.getErrors().get( 0 );
			}
			/**
			  new XmlDocumentImpl( cn/java/hibernate/entity/student.hbm.xml文件的document,"resource", "cn/java/hibernate/entity/student.hbm.xml" );
			 */
