...
public class XmlConfiguration
{
    ...
    {
        return _processor.configure(obj); // org.eclipse.jetty.xml.XmlConfiguration.JettyXmlConfiguration
    }
    ...
    private static class JettyXmlConfiguration implements ConfigurationProcessor
    {
        ...
        public void init(URL url, XmlParser.Node root, XmlConfiguration configuration)
        {
            ...
            _root=root; // <Configure>节点
            _configuration=configuration;
        }

        public Object configure(Object obj) throws Exception
        {
        	// obj=== org.eclipse.jetty.server.Server的子类
        ...
        private static Class<?> nodeClass(XmlParser.Node node) throws ClassNotFoundException
        {
            ...
            return Loader.loadClass(XmlConfiguration.class,className); // 加载<... class="" >标签上配置的类
        }
        ...
        {
        	// obj=== org.eclipse.jetty.server.Server的实例或者子类实例
        	// cfg 是  <Configure>节点
        	// _configuration === org.eclipse.jetty.xml.XmlConfiguration
                ...
                try
                {
                    ...
                    switch (tag)
                    {
                        case "Set":
                            ...
                            break;
                        case "Put":
                            put(obj, node); // obj.put(node.getAttribute("name"),value);
                            break;
                        case "Call":
                            call(obj, node);
                            break;
        ...
        private void set(Object obj, XmlParser.Node node) throws Exception
        {
            ...
            try
            {
                Method set = oClass.getMethod(name,vClass); // 获取指定类型的setter方法
                set.invoke(obj,arg);  // 通过setter方法进行注入
                return;
            }
            ...
            try
            {
                ...
                Method set = oClass.getMethod(name,vClass); // 通过字段进行注入
                set.invoke(obj,arg);
                return;
            }
            ...
            for (int s = 0; sets != null && s < sets.length; s++)
            {
                ...
                if (name.equals(sets[s].getName()) && paramTypes.length == 1)
                {
                    ...
                    {
                        set = sets[s];
                        sets[s].invoke(obj,arg); // 通过setter方法进行注入
                        return;
                    }
        ...
        private Object call(Object obj, XmlParser.Node node) throws Exception
        {
            ...
            if (oClass != null)
                ...
                oClass = obj.getClass(); // org.eclipse.jetty.server.Server
            if (oClass == null)
