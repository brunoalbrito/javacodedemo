...
public class Server extends HandlerWrapper implements Attributes
{
    ...
    protected void doStart() throws Exception
    {
        ...
        if (getStopAtShutdown())
            ShutdownThread.register(this); // 添加Runtime.getRuntime().addShutdownHook(...)处理器

        //Register the Server with the handler thread for receiving
        //remote stop commands
        ShutdownMonitor.register(this); // 注册“关闭监听器”
        
        //Start a thread waiting to receive "stop" commands.
        ShutdownMonitor.getInstance().start(); // initialize  启动线程
        ...
        SizedThreadPool pool = getBean(SizedThreadPool.class); // 获取线程池 org.eclipse.jetty.util.thread.QueuedThreadPool
        int max=pool==null?-1:pool.getMaxThreads(); // 最大线程数
        int selectors=0;
        int acceptors=0;
        if (mex.size()==0) // 没有异常
        {
            for (Connector connector : _connectors)
            {
                if (connector instanceof AbstractConnector)
                    acceptors+=((AbstractConnector)connector).getAcceptors(); // “接受连接”的线程
                    
                if (connector instanceof ServerConnector)
                    selectors+=((ServerConnector)connector).getSelectorManager().getSelectorCount(); // “消费连接”的线程
            }
        }
    ...
    {
        getThreadPool().join(); // org.eclipse.jetty.util.thread.QueuedThreadPool
    }
