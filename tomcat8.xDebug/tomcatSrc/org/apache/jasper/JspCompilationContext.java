...
public class JspCompilationContext {
    ...
    private JspCompilationContext(String jspUri, TagInfo tagInfo,
        ...
        this.rctxt = rctxt;
        this.basePackageName = Constants.JSP_PACKAGE_NAME; // org.apache.jsp默认包名
    ...
    public String getOutputDir() {
        ...
        // outputDir === d:/a/b/tomcat/work/Catalina/localhost/test/org/apache/jsp
    ...
    public Compiler createCompiler() {
        ...
        if (options.getCompilerClassName() != null) {
            ...
            if (options.getCompiler() == null) {// 走这里
                jspCompiler = createCompiler("org.apache.jasper.compiler.JDTCompiler"); // 创建编译器 !!!这里
                if (jspCompiler == null) {
                    jspCompiler = createCompiler("org.apache.jasper.compiler.AntCompiler"); // 创建编译器
                }
            } else {
    ...
    public String getServletClassName() {
        ...
        if (isTagFile) {
            ...
            className = JspUtil.makeJavaIdentifier(jspUri.substring(iSep)); // 把文件名转换作为类名 index.jsp ---> index_jsp
        }
        return className;
    }
    ...
    public String getServletPackageName() {
        if (isTagFile()) {
            ...
            if (dPackageName.length() == 0) {
                return basePackageName; // org.apache.jsp
            }
            return basePackageName + '.' + getDerivedPackageName(); // org.apache.jsp.dir1
        }
    }
    ...
    protected String getDerivedPackageName() {
        if (derivedPackageName == null) {
            ...
            derivedPackageName = (iSep > 0) ?
                    JspUtil.makeJavaPackage(jspUri.substring(1,iSep)) : ""; // 目录级别
        }
    ...
    public String getClassFileName() {
        if (classFileName == null) {
        	// getOutputDir() === d:/a/b/tomcat/work/Catalina/localhost/test/org/apache/jsp
        	// getServletClassName() === 把文件名转换作为类名 index.jsp ---> index_jsp
        	// classFileName === d:/a/b/tomcat/work/Catalina/localhost/test/org/apache/jsp/index_jsp.class
    ...
    public TldResourcePath getTldResourcePath(String uri) {
    	// 在 org.apache.jasper.EmbeddedServletOptions 构造函数设置了TldCache
    	// getOptions() == org.apache.jasper.EmbeddedServletOptions
    	// TldCache == org.apache.jasper.compiler.TldCache
    	// org.apache.tomcat.util.descriptor.tld.TldResourcePath
    	// return TldCache.getInstance(ApplicationContextFacade).getTldResourcePath(uri);
    ...
    public void compile() throws JasperException, FileNotFoundException {
        ...
        // jspCompiler === org.apache.jasper.compiler.JDTCompiler
        // ctxt.getClassFileName()
        // ctxt.getClassFileName() === d:/a/b/tomcat/work/Catalina/localhost/test/org/apache/jsp/index_jsp.class
        if (jspCompiler.isOutDated()) { //!!!! class文件是否已经过期
            if (isRemoved()) {
                throw new FileNotFoundException(jspUri);
            }
            try {
            	// ---------------------- 
                jspCompiler.removeGeneratedFiles(); // 删除编译的  .class 和  .java文件
                jspLoader = null;
                // ---------------------- 
                jspCompiler.compile(); // 把jsp转成Java文件，把Java文件编译成class文件
                jsw.setReload(true);
    ...
    protected void createOutputDir() {
        ...
        try {
        	// options === org.apache.jasper.EmbeddedServletOptions
            File base = options.getScratchDir(); // Tomcat的临时目录，或者系统的临时目录
            // base === d:/a/b/tomcat/work/Catalina/localhost/test
            // path === org/apache/jsp
            baseUrl = base.toURI().toURL();
            // outputDir === d:/a/b/tomcat/work/Catalina/localhost/test/org/apache/jsp
            outputDir = base.getAbsolutePath() + File.separator + path +
                    File.separator;
            if (!makeOutputDir()) { // d:/a/b/tomcat/work/Catalina/localhost/test/org/apache/jsp
