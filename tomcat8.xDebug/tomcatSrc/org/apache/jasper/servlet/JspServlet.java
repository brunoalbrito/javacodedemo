...
public class JspServlet extends HttpServlet implements PeriodicEventListener {
    ...
    public void init(ServletConfig config) throws ServletException {
        ...
        super.init(config);
        this.config = config; // org.apache.catalina.core.StandardWrapperFacade
        ...
        if (config.getInitParameter("jspFile") != null) { // jsp文件 ，可通过<jsp-file>配置
            jspFile = config.getInitParameter("jspFile"); // jspFile == "/a/b/c/file.jsp"
            try {
            	// context == org.apache.catalina.core.ApplicationContextFacade
            	// org.apache.catalina.core.ApplicationContextFacade.getResource(jspFile)
            	// org.apache.catalina.core.ApplicationContext.getResource(jspFile)
            	// org.apache.catalina.webresources.StandardRoot.getResource(jspFile).getURL();
                if (null == context.getResource(jspFile)) { // 检查JSP文件是否存在
                    return;
                }
    ...
    private void serviceJspFile(HttpServletRequest request,
        ...
        if (wrapper == null) {
            synchronized(this) {
                wrapper = rctxt.getWrapper(jspUri); // 再取一次
                if (wrapper == null) {
