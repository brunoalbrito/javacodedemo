...
public class PageContextImpl extends PageContext {
	...
	// jspf == org.apache.jasper.runtime.JspFactoryImpl
    ...
    public void initialize(Servlet servlet, ServletRequest request,
        ...
        this.config = servlet.getServletConfig(); // org.apache.catalina.core.StandardWrapperFacade
        this.context = config.getServletContext(); // org.apache.catalina.core.ApplicationContextFacade
        this.errorPageURL = errorPageURL;
        this.request = request;
        this.response = response;

        // initialize application context
        this.applicationContext = JspApplicationContextImpl.getInstance(context); //!!! org.apache.jasper.runtime.JspApplicationContextImpl
    ...
    public static Object proprietaryEvaluate(final String expression,
            ...
            throws ELException {
    	// 处理EL表达式
    	// expression = "${requestScope.username}"
    	// expectedType = java.lang.String.class;
    	...
    	// 通过JSP工厂取得JSP应用上下文
    	// 通过JSP应用上下文取得表达式工厂
    	// exprFactory == org.apache.jasper.runtime.JspFactoryImpl.getJspApplicationContext(pageContext.getServletContext()).getExpressionFactory();
        // exprFactory == org.apache.jasper.runtime.JspApplicationContextImpl.getExpressionFactory();
    	// exprFactory == org.apache.el.ExpressionFactoryImpl
    	// jspf.getJspApplicationContext() === new JspApplicationContextImpl()
    	// exprFactory === new org.apache.el.ExpressionFactoryImpl()
    	final ExpressionFactory exprFactory = jspf.getJspApplicationContext(pageContext.getServletContext()).getExpressionFactory();
        // ctx === org.apache.jasper.el.ELContextImpl
        ...
        // exprFactory == org.apache.el.ExpressionFactoryImpl
        ValueExpression ve = exprFactory.createValueExpression(ctx, expression, expectedType); // 解析EL表达式成对象树
        // ve === org.apache.el.ValueExpressionImpl
        return ve.getValue(ctx); // 取得指定表达式的值
    }

    @Override
    public ELContext getELContext() {
        if (elContext == null) {
        	// !!! applicationContext === org.apache.jasper.runtime.JspApplicationContextImpl
        	// elContext === org.apache.jasper.el.ELContextImpl
            elContext = applicationContext.createELContext(this); // 创建 EL 上下文 
            if (servlet instanceof JspSourceImports) {//!!!
                ImportHandler ih = elContext.getImportHandler(); // javax.el.ImportHandler
                Set<String> packageImports = ((JspSourceImports) servlet).getPackageImports();
                if (packageImports != null) {
                    for (String packageImport : packageImports) {
                        ih.importPackage(packageImport);  // 添加要导入的包
                    }
                }
                Set<String> classImports = ((JspSourceImports) servlet).getClassImports();
                if (classImports != null) {
                    for (String classImport : classImports) {
                        ih.importClass(classImport); // 添加要导入的类
                    }
                }
            }
        }
