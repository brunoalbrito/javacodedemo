...
class ParserController implements TagConstants {
    ...
    public Node.Nodes parse(String inFileName, Node parent, Jar jar)
        ...
        // remain unchanged.
    	// 如： parent == org.apache.jasper.compiler.Node.IncludeDirective
    ...
    public Node.Nodes parseTagFileDirectives(String inFileName, Jar jar)
            throws JasperException, IOException {
        boolean isTagFileSave = isTagFile; // 保存现场
        isTagFile = true; // 是标签文件
        boolean directiveOnlySave = directiveOnly; // 保存现场
        directiveOnly = true; // 值解析指令
        Node.Nodes page = doParse(inFileName, null, jar);
        
        directiveOnly = directiveOnlySave; // 还原现场
        isTagFile = isTagFileSave; // 还原现场
        return page;
    }
    ...
    private Node.Nodes doParse(String inFileName, Node parent, Jar jar)
        ...
        // dealing with
        determineSyntaxAndEncoding(absFileName, jar, jspConfigPageEnc); // 检测识别文件的编码 jsp文件的编码类型
        
        // 如： parent == org.apache.jasper.compiler.Node.IncludeDirective   <jsp:directive.include></jsp:directive.include>
        if (parent != null) {
