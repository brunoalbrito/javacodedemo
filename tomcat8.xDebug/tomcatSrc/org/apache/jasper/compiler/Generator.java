...
class Generator {
    ...
    private void declareTemporaryScriptingVars(Node.Nodes page)
        ...
        class ScriptingVarVisitor extends Node.Visitor {
            ...
            public void visit(Node.CustomTag n) throws JasperException {
                ...
                if (n.getCustomNestingLevel() > 0) { // 嵌套深度
    ...
    private void genPreambleMethods() {
        ...
        generateGetters(); // 生成getter方法  _jsp_getExpressionFactory
        generateInit(); // 生成初始方法   _jspInit
        generateDestroy(); // 生成析构函数 _jspDestroy
    }
    ...
    private void generatePreamble(Node.Nodes page) throws JasperException {
        ...
        String servletPackageName = ctxt.getServletPackageName();  // org.apache.jsp   org.apache.jsp.dir1
        String servletClassName = ctxt.getServletClassName(); // index_jsp
        String serviceMethodName = Constants.SERVICE_METHOD_NAME; // _jspService

        // First the package name:
        genPreamblePackage(servletPackageName);  // 生成包名 package org.apache.jsp.dir1;

        // Generate imports
        genPreambleImports(); // 生成要导入的包

        // Generate class declaration  生成类声明
        ...
        genPreambleStaticInitializers(); // 静态初始化块

        // Class variable declarations 
        genPreambleClassVariableDeclarations(); // 声明类变量

        // Methods here
        genPreambleMethods();  // 生成各种方法

        // Now the service method // 生成 _jspService方法
        ...
        // Local variable declarations  本地变量
        ...
        declareTemporaryScriptingVars(page); // 生成临时变量
        out.println();
    ...
    private class GenerateVisitor extends Node.Visitor {
        ...
        public void visit(Node.Expression n) throws JasperException {
            ...
            out.printMultiLn(n.getText()); // 表达式
            out.println(");");
            n.setEndJavaLine(out.getJavaLine());
        }
...
//        	<!-- 动态引入行为 -->
//        	<jsp:include page="/WEB-INF/jsp/common/menu.jsp" flush="true">
//        	     <jsp:param name="param1" value="value1" />
//        	</jsp:include>
        	
            String flush = n.getTextAttribute("flush");
            Node.JspAttribute page = n.getPage(); // include文件
...
//            handlerInfo {
//        		"tagHandlerClass" : "cn.java.handler.Tag1"
//        	}
            TagHandlerInfo handlerInfo = getTagHandlerInfo(n);

            // Create variable names
            String baseVar = createTagVarName(n.getQName(), n.getPrefix(), n
                    .getLocalName()); // myprefix_tag1_0
	         // tagEvalVar = "_jspx_eval_myprefix_tag1_0"
	         // tagHandlerVar = "_jspx_th_myprefix_tag1_0"
	         // tagPushBodyCountVar = "_jspx_push_body_count_myprefix_tag1_0"
            ...
            if (n.implementsSimpleTag()) {
                 ...
                 */
            	// 在java文件写入如下代码
            	// cn.java.handler.Tag1 _jspx_th_myprefix_tag1_0 = (new cn.java.handler.Tag1());
            	// _jsp_getInstanceManager().newInstance(_jspx_th_myprefix_tag1_0);
            	// _jspx_th_myprefix_tag1_0.setPageContext(_jspx_page_context);
            	// _jspx_th_myprefix_tag1_0.setParent(parent);
                // _jspx_th_myprefix_tag1_0.setDynamicAttribute("uri1","value1");
            	// _jspx_th_myprefix_tag1_0.setJspId("createJspId");
            	// int _jspx_eval_myprefix_tag1_0 = _jspx_th_myprefix_tag1_0.doStartTag();
                generateCustomStart(n, handlerInfo, tagHandlerVar, tagEvalVar,
                        tagPushBodyCountVar); // 开始代码
                ...
                // if (_jspx_th_myprefix_tag1_0.doEndTag() == javax.servlet.jsp.tagext.Tag.SKIP_PAGE) {
                // 		return;
                // }
                // _jspx_th_myprefix_tag1_0.release();
...
//                	}
    ...
    public static void generate(ServletWriter out, Compiler compiler,
        ...
        gen.generateCommentHeader(); // 写入注释
        if (gen.ctxt.isTagFile()) { // 如果是标签文件
            ...
            gen.generatePreamble(page); // 写入半部分代码
            gen.generateXmlProlog(page); // 写入 HTML 头
            gen.fragmentHelperClass.generatePreamble();
            page.visit(gen.new GenerateVisitor(gen.ctxt.isTagFile(), out,
                    gen.methodsBuffered, gen.fragmentHelperClass)); // 写入主题内容
            gen.generatePostamble(); // 写入后半部分代码
        }
    }
    ...
    private static class TagHandlerInfo {
        ...
        TagHandlerInfo(Node n, Class<?> tagHandlerClass,
            ...
            try {
                ...
                PropertyDescriptor[] pd = tagClassInfo.getPropertyDescriptors(); // 反射处理类的属性
                for (int i = 0; i < pd.length; i++) {
                    /*
                     * FIXME: should probably be checking for things like
                     * pageContext, bodyContent, and parent here -akv
                     */
                    if (pd[i].getWriteMethod() != null) { // 可以写
