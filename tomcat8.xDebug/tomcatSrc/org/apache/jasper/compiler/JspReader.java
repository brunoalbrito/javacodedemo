...
class JspReader {
    ...
    public JspReader(JspCompilationContext ctxt,
        ...
        try {
            ...
            for (int i = 0 ; (i = reader.read(buf)) != -1 ;)
                caw.write(buf, 0, i);
            caw.close();
            current = new Mark(this, caw.toCharArray(), fname); /// !!! 文件内容
        } catch (Throwable ex) {
    ...
    Mark skipELExpression() {
        ...
        int currentChar;
        do {
            currentChar = nextChar(last); // 读取下一个字符
            ...
            if (currentChar == '"' && !singleQuoted) {  // 出现双引号
                doubleQuoted = !doubleQuoted;
            } else if (currentChar == '\'' && !doubleQuoted) { // 出现单引号
                singleQuoted = !singleQuoted;
            } else if (currentChar == '{' && !doubleQuoted && !singleQuoted) { // 嵌套
                nesting++;
