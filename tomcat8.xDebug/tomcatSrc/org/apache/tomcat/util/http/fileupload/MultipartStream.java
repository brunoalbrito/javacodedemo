...
public class MultipartStream {
    ...
    public byte readByte() throws IOException {
        ...
        if (head == tail) {
            head = 0;
            // Refill.
            // input == org.apache.tomcat.util.http.fileupload.util.LimitedInputStream
            tail = input.read(buffer, head, bufSize); // 读取内容
            if (tail == -1) {
    ...
    public boolean readBoundary()
        ...
        head += boundaryLength;
        try {
        	// 如: marker == new byte[2]{"a=value1","CRLF"}
            marker[0] = readByte(); // 读取内容
            if (marker[0] == LF) {
            ...
            marker[1] = readByte();
            if (arrayequals(marker, STREAM_TERMINATOR, 2)) { // 字节的分割符
                nextChunk = false;
            } else if (arrayequals(marker, FIELD_SEPARATOR, 2)) { // 字段的分割符
                nextChunk = true;
            } else {
...
//		
        ...
        if (headerEncoding != null) {
            try {
                headers = baos.toString(headerEncoding); // 转码
    ...
    public boolean skipPreamble() throws IOException {
        ...
        try {
            ...
            // encapsulation.
            return readBoundary(); // 读取内容  
        } catch (MalformedStreamException e) {
            return false;
        } finally {
            // Restore delimiter.
            System.arraycopy(boundary, 0, boundary, 2, boundary.length - 2); // 还原回来
