...
public abstract class FileUploadBase {
    ...
    public List<FileItem> parseRequest(RequestContext ctx)
        ...
        try {
            ...
            while (iter.hasNext()) {
            	// item === org.apache.tomcat.util.http.fileupload.FileUploadBase.FileItemIteratorImpl.FileItemStreamImpl
                final FileItemStream item = iter.next(); // 下一个表单项
                // Don't use getName() here to prevent an InvalidFileNameException.
                final String fileName = ((FileItemIteratorImpl.FileItemStreamImpl) item).name;
                // fac == org.apache.tomcat.util.http.fileupload.disk.DiskFileItemFactory
                FileItem fileItem = fac.createItem(item.getFieldName(), item.getContentType(),
                                                   item.isFormField(), fileName);
                // fileItem == org.apache.tomcat.util.http.fileupload.disk.DiskFileItem 表单项信息
                items.add(fileItem); // 添加项
                try {
                    Streams.copy(item.openStream(), fileItem.getOutputStream(), true); // 拷贝文件内容
                } catch (FileUploadIOException e) {
                    throw (FileUploadException) e.getCause();
                } catch (IOException e) {
                ...
                fileItem.setHeaders(fih); // 表单项的头信息
            }
            successful = true;
            return items;
        } catch (FileUploadIOException e) {
    ...
    protected String getFieldName(FileItemHeaders headers) {
    	// headers == org.apache.tomcat.util.http.fileupload.util.FileItemHeadersImpl
        return getFieldName(headers.getHeader(CONTENT_DISPOSITION)); // Content-disposition
    }
...
//    	pContentDisposition === 'form-data; name="lname"'
//    	pContentDisposition === 'form-data; form-data; name="file1"; filename="1.png'
...
//    	如:
//			headerPart == 'Content-Disposition: form-data; name="lname"';
//    	如:
//    		headerPart == 'Content-Disposition: form-data; name="file1"; filename="1.png"
//    		Content-Type: image/png';
        ...
        for (;;) {
            ...
            //header == 'Content-Disposition: form-data; name="lname"';
            //header == 'Content-Disposition: form-data; name="file1"; filename="1.png"';
    ...
    private void parseHeaderLine(FileItemHeadersImpl headers, String header) {
        ...
        String headerName = header.substring(0, colonOffset).trim(); // Content-Disposition
        String headerValue =
            header.substring(header.indexOf(':') + 1).trim(); // form-data; name="lname"
    ...
    private class FileItemIteratorImpl implements FileItemIterator {
        ...
        class FileItemStreamImpl implements FileItemStream {
            ...
            FileItemStreamImpl(String pName, String pFieldName,
                ...
                final ItemInputStream itemStream = multi.newInputStream(); // 文件内容，记录文件内容的流的位置
        ...
        FileItemIteratorImpl(RequestContext ctx)
                throws FileUploadException, IOException {
        	// !! ctx === org.apache.tomcat.util.http.fileupload.servlet.ServletRequestContext
            if (ctx == null) {
                throw new NullPointerException("ctx parameter");
            }

            String contentType = ctx.getContentType(); // 内容类型
            if ((null == contentType)
            ...
            final long requestSize = ((UploadContext) ctx).contentLength(); // 内容大小

            InputStream input; // N.B. this is eventually closed in MultipartStream processing
            if (sizeMax >= 0) {
                if (requestSize != -1 && requestSize > sizeMax) { // 超过允许的body大小
                ...
                input = new LimitedInputStream(ctx.getInputStream(), sizeMax) { // 这里
                    @Override
                    protected void raiseError(long pSizeMax, long pCount)
                            throws IOException {
            ...
            if (charEncoding == null) {
                charEncoding = ctx.getCharacterEncoding(); // 取得编码类型
            }
            // content-type = "multipart/form-data; boundary=---------------------------173751090716589"
            // boundary = "---------------------------173751090716589"
            boundary = getBoundary(contentType); //取得边界
            if (boundary == null) {
                IOUtils.closeQuietly(input); // avoid possible resource leak
                throw new FileUploadException("the request was rejected because no multipart boundary was found");
            }

            notifier = new MultipartStream.ProgressNotifier(listener, requestSize); // 通知器
            try {
                multi = new MultipartStream(input, boundary, notifier); // 创建对象
...
//        		�PNG
//        		
                FileItemHeaders headers = getParsedHeaders(multi.readHeaders()); // 读取每个part的头
//          如:
//    			multi.readHeaders() == 'Content-Disposition: form-data; name="lname"';
//        	如:
//        		multi.readHeaders() == 'Content-Disposition: form-data; name="file1"; filename="1.png"
//        		Content-Type: image/png';
                if (currentFieldName == null) {
//                	headers = FileItemHeadersImpl{
//                		"Content-Disposition" : 'form-data; name="lname"'
//                	}
//                	headers = FileItemHeadersImpl{
//                		"Content-Disposition" : 'form-data; form-data; name="file1"; filename="1.png',
//                		"Content-Type" : 'image/png'
//                	}
                    ...
                    if (fieldName != null) {
                        String subContentType = headers.getHeader(CONTENT_TYPE); // 有声明内容类型
                        if (subContentType != null
                                &&  subContentType.toLowerCase(Locale.ENGLISH)
                                        .startsWith(MULTIPART_MIXED)) { // "multipart/mixed"
