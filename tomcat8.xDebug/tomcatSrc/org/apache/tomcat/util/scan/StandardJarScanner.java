...
public class StandardJarScanner implements JarScanner {
    ...
    public void scan(JarScanType scanType, ServletContext context,
        ...
        // Scan WEB-INF/lib
        Set<String> dirList = context.getResourcePaths(Constants.WEB_INF_LIB); // "/WEB-INF/lib/"
        if (dirList != null) {
        ...
        if (isScanClassPath()) {
            ...
            ClassLoader classLoader = context.getClassLoader(); // 上下文类加载器
    ...
    private void process(JarScanType scanType, JarScannerCallback callback,
        ...
        if ("jar".equals(url.getProtocol()) || url.getPath().endsWith(Constants.JAR_EXT)) {
            try (Jar jar = JarFactory.newInstance(url)) { // jar包对象
                processManifest(jar, isWebapp, classPathUrlsToProcess);
                callback.scan(jar, webappPath, isWebapp); // org.apache.jasper.servlet.TldScanner.TldScannerCallback
            }
            ...
            try {
                ...
                if (f.isFile() && isScanAllFiles()) {
                    ...
                    URL jarURL = UriUtil.buildJarUrl(f); //把文件当成JAR包扫描一下
    ...
    private static void processManifest(Jar jar, boolean isWebapp,
        ...
        if (manifest != null) {
            ...
            for (String classPathEntry : classPathEntries) {  // 多类路径
