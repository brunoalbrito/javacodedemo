...
//          设置@PostConstruct/@PreDestroy类型的资源依赖
    ...
    protected void postConstruct(Object instance, final Class<?> clazz)
        ...
        if (superClass != Object.class) {
            postConstruct(instance, superClass); // 在父类中查找
        }
                    ...
                    for (Field field : fields) {
                        ...
                        if (injections != null && injections.containsKey(field.getName())) { // 特殊注入
                            annotations.add(new AnnotationCacheEntry(
                                ...
                                field.getAnnotation(Resource.class)) != null) { // 字段有声明资源的注解 @Resource
                            annotations.add(new AnnotationCacheEntry(field.getName(), null,
                                ...
                                field.getAnnotation(EJB.class)) != null) { // ejb的注解  @EJB
                            annotations.add(new AnnotationCacheEntry(field.getName(), null,
                                    ejbAnnotation.name(), AnnotationCacheEntryType.FIELD));
                        } else if ((webServiceRefAnnotation =
                                field.getAnnotation(WebServiceRef.class)) != null) { //注解 @WebServiceRef
                            annotations.add(new AnnotationCacheEntry(field.getName(), null,
                                    webServiceRefAnnotation.name(),
                                    AnnotationCacheEntryType.FIELD));
                        } else if ((persistenceContextAnnotation =
                                field.getAnnotation(PersistenceContext.class)) != null) { //注解 @PersistenceContext
                            annotations.add(new AnnotationCacheEntry(field.getName(), null,
                                    persistenceContextAnnotation.name(),
                                    AnnotationCacheEntryType.FIELD));
                        } else if ((persistenceUnitAnnotation =
                                field.getAnnotation(PersistenceUnit.class)) != null) { //注解 @PersistenceUnit
                            annotations.add(new AnnotationCacheEntry(field.getName(), null,
