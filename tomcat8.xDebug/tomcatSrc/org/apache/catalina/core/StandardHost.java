...
public class StandardHost extends ContainerBase implements Host {
    ...
    public File getConfigBaseFile() {
        ...
        if (getXmlBase()!=null) {
            ...
            path = xmlDir.toString();// d:/a/b/tomcat/conf/Catalina/localhost
        }
        File file = new File(path);
        if (!file.isAbsolute())
            file = new File(getCatalinaBase(), path); //绝对路径
        try {
            file = file.getCanonicalFile();
...
//        org.apache.catalina.core.StandardContext 添加子节点，启动子节点
//        org.apache.catalina.startup.ContextConfig  触发子节点的事件
        super.addChild(child); // 添加子节点
