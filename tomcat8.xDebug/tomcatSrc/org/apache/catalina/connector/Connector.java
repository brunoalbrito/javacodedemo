...
public class Connector extends LifecycleMBeanBase  {
    ...
    public Connector(String protocol) {
        setProtocol(protocol);  //  HTTP/1.1      AJP/1.3
...
//    	如：name = address
//    	address="127.0.0.1" 是通过这个渠道设置的
        String repl = name;
        if (replacements.get(name) != null) {
            repl = replacements.get(name);
        }
        return IntrospectionUtils.setProperty(protocolHandler, repl, value); // 反射设置属性
    }
...
//      protocolHandler =  org.apache.coyote.http11.Http11NioProtocol
        ...
        if( null == parseBodyMethodsSet ) { // 设置要解析body的请求方法
            setParseBodyMethods(getParseBodyMethods()); // POST
        }
        ...
        try {
        	// 内部会调用 protocolHandler.endpoint.init(...)，会创建 serverSock
//        	org.apache.tomcat.util.net.NioEndpoint.bind(...)
            protocolHandler.init(); // org.apache.coyote.http11.Http11NioProtocol
        } catch (Exception e) {
            throw new LifecycleException(
