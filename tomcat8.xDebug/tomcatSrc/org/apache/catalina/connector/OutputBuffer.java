...
public class OutputBuffer extends Writer
    ...
    private void setConverter() throws IOException {
        ...
        if (coyoteResponse != null) {
            enc = coyoteResponse.getCharacterEncoding(); // 编码类型
        }
        ...
        if (conv == null) {
        	// org.apache.tomcat.util.buf.C2BConverter("utf-8")
            conv = createConverter(charset); // 创建编码转换器
    ...
    private static C2BConverter createConverter(final Charset charset) throws IOException {
        if (Globals.IS_SECURITY_ENABLED){
            ...
            return new C2BConverter(charset); // 编码转换器
        }
    }
    ...
    public void reset(boolean resetWriterStreamFlags) {
        bb.recycle(); // 清除数据,回收空间
        cb.recycle();
        bytesWritten = 0;
        charsWritten = 0;
        if (resetWriterStreamFlags) {
            if (conv != null) {
                conv.recycle(); // conv == org.apache.tomcat.util.buf.C2BConverter("utf-8")
            }
            conv = null;
            enc = null;
        }
