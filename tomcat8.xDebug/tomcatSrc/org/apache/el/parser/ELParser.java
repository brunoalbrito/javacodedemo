...
public class ELParser/* @bgen(jjtree) */implements ELParserTreeConstants, ELParserConstants {/*
																							 * @
																							 * bgen
																							 * (
																							 * jjtree
	...
	final public AstCompositeExpression CompositeExpression() throws ParseException {
		...
		jjtree.openNodeScope(jjtn000); // 打开节点作用域  jjtn000.jjtOpen();
		try {
			label_1: while (true) {
				// 以 #{ 开头, jj_ntk = 3
				// 以 ${ 开头, jj_ntk = 2
				switch ((jj_ntk == -1) ? jj_ntk() : jj_ntk) { // 读取下一个token
				case LITERAL_EXPRESSION: // 1
				case START_DYNAMIC_EXPRESSION: // 2
				case START_DEFERRED_EXPRESSION: // 3
					;
					break;
				default:
					jj_la1[0] = jj_gen; //!!!
				...
				switch ((jj_ntk == -1) ? jj_ntk() : jj_ntk) { // 如果没有token,就取得下一个token 
				case START_DEFERRED_EXPRESSION: // #{...
					DeferredExpression(); // 递延表达式  
					break;
			...
			jj_consume_token(0); // 消费掉token
			jjtree.closeNodeScope(jjtn000, true); // 关闭节点作用域  jjtn000.jjtClose();
			jjtc000 = false;
			{
				if (true)
					return jjtn000;//!!!
	...
	final public void DynamicExpression() throws ParseException {
		...
		boolean jjtc000 = true;
		jjtree.openNodeScope(jjtn000); //  jjtn000.jjtOpen();
		try {
			...
			if (jjtc000) {
				jjtree.closeNodeScope(jjtn000, true); // jjtn000.jjtClose();
			}
		}
	}
	 ...
	 * but being bracketed does not mean it is an invocation.
	 */
	 ...
	 * required to convert it to an empty Map when appropriate.
	 */
	...
	private static void jj_la1_init_0() {
		jj_la1_0 = new int[] { 0xe, 0xe, 0x800000, 0x15ed00, 0x15ed00, 0x1000000, 0x0, 0x40000, 0x15ed00, 0x40000, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0xfe000000, 0x18000000, 0x6000000,
				0x80000000, 0x60000000, 0xfe000000, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x15ed00, 0x120000, 0x15ed00, 0x120000, 0x40000, 0x1000000, 0x15ed00, 0x40000, 0x0, 0x100100, 0x1000000,
				0x15ed00, 0x1000000, 0x15ed00, 0x1000000, 0x15ed00, 0x400000, 0x40000, 0x1ec00, 0xc000, };
	}

	private static void jj_la1_init_1() {
		jj_la1_1 = new int[] { 0x0, 0x0, 0x0, 0x1008860, 0x1008860, 0x0, 0x1000000, 0x1000000, 0x1008860, 0x0, 0x600, 0x600, 0x180, 0x180, 0x1e, 0x6, 0x18, 0x1e, 0x1, 0x0, 0x0, 0x1, 0x0, 0x1,
				0x200000, 0xc000, 0xc000, 0x1e2000, 0x60000, 0x180000, 0x1e2000, 0x60, 0x1008860, 0x0, 0x1000000, 0x0, 0x0, 0x0, 0x1008860, 0x0, 0x1000000, 0x0, 0x0, 0x1008860, 0x0, 0x1008860, 0x0,
	...
	// 读取下一个token
	private int jj_ntk() {
		//	  token_source = new ELParserTokenManager(SimpleCharStream(new StringReader("${requestScope.username}"), 1, 1));
		//    token = new Token();
		// token_source.getNextToken() = new Token(3, "#{");
		// token_source.getNextToken() =new Token(2, "${");
		// Token链条
