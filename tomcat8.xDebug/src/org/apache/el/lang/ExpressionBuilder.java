...
public final class ExpressionBuilder implements NodeVisitor {
    ...
    private static final Node createNodeInternal(String expr)
        ...
        if (n == null) {
            ...
            try {
                if (parser == null) {
                	// new StringReader("${requestScope.username}")
                    parser = new ELParser(new StringReader(expr)); // 创建 EL表达式解析器
                } else {
                    parser.ReInit(new StringReader(expr));
                }
                n = parser.CompositeExpression(); // 复合表达式
...
//                n [AstDynamicExpression] = {
//                	children : {
//	                	[0]	AstIdentifier : {
//	                		"children" : null,
//	                		"id" : 35,
//	                		"image" : "applicationScope"
//	                		"parent" : AstValue
//	                	},
//	                	[1]	AstDotSuffix : {
//	                		"children" : null,
...
//                }
    ...
    private Node build() throws ELException {
    	// this.expression == "${requestScope.username}"
        Node n = createNodeInternal(this.expression); // 创建节点
        this.prepare(n);
        if (n instanceof AstDeferredExpression
                || n instanceof AstDynamicExpression) {
            n = n.jjtGetChild(0);
