...
public final class StandardServer extends LifecycleMBeanBase implements Server {
    ...
    protected void initInternal() throws LifecycleException {
        ...
        onameStringCache = register(new StringCache(), "type=StringCache"); // 把StringCache注册到 MBeanServer服务器中

        // Register the MBeanFactory
        MBeanFactory factory = new MBeanFactory();
        factory.setContainer(this);
        onameMBeanFactory = register(factory, "type=MBeanFactory"); // 把MBeanFactory注册到 MBeanServer服务器中

        // Register the naming resources
        globalNamingResources.init(); // 注册全局命名资源

        // Populate the extension validator with JARs from common and shared
        // class loaders
        if (getCatalina() != null) {
            ClassLoader cl = getCatalina().getParentClassLoader(); // 父类加载器
                        ...
                        if (url.getProtocol().equals("file")) {
                            try {
                                ...
                                if (f.isFile() &&
                                    ...
                                    ExtensionValidator.addSystemResource(f); // 读取jar包的摘要信息
                                }
