...
public class StandardService extends LifecycleMBeanBase implements Service {
    ...
    public void addConnector(Connector connector) {
        ...
        synchronized (connectorsLock) {
            connector.setService(this); // 设置依赖
    ...
    protected void startInternal() throws LifecycleException {
        ...
        if (engine != null) {
            synchronized (engine) {
                engine.start(); // !!!org.apache.catalina.core.StandardEngine
            }
        }
    ...
    protected void initInternal() throws LifecycleException {
        ...
        // Initialize any Executors 执行执行器
        ...
        mapperListener.init(); // 把MapperListener对象注册到MBeanServer服务器中

        // Initialize our defined Connectors
        synchronized (connectorsLock) {
            for (Connector connector : connectors) { // 初始化连接器
                try {
                	// 内部最终会创建serverSock对象，进行监听 org.apache.tomcat.util.net.NioEndpoint.bind(...);
                    connector.init();//!!!
                } catch (Exception e) {
