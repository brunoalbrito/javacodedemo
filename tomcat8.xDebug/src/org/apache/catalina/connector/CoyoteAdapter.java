...
public class CoyoteAdapter implements Adapter {
    ...
    protected boolean postParseRequest(org.apache.coyote.Request req, Request request,
        ...
        // processor hasn't set it, use the settings from the connector  解析出scheme类型
        ...
        // Filter trace method
        if (!connector.getAllowTrace()
                && req.method().equalsIgnoreCase("TRACE")) { // TRACE请求方式，暴露Servlet的所有方法
    ...
    protected void parseSessionCookiesId(Request request) {
        ...
        if (context != null && !context.getServletContext()
                .getEffectiveSessionTrackingModes().contains(
                        SessionTrackingMode.COOKIE)) { // 如果sessionID不是从URL中拿，直接返回
            return;
        }
        ...
        String sessionCookieName = SessionConfig.getSessionCookieName(context); // session存在cookie的键名  ，如：JSESSIONID

        for (int i = 0; i < count; i++) {
            ServerCookie scookie = serverCookies.getCookie(i);
            if (scookie.getName().equals(sessionCookieName)) {
                // Override anything requested in the URL
                if (!request.isRequestedSessionIdFromCookie()) { // 没解析过sessionID
                    // Accept only the first session id cookie
                    convertMB(scookie.getValue());
                    request.setRequestedSessionId
                        (scookie.getValue().toString()); // 设置sessionID的值
                    request.setRequestedSessionCookie(true); // 说明从cookie来的sessionID
                    request.setRequestedSessionURL(false); // 说明不是从cookie来的sessionID
                    if (log.isDebugEnabled()) {
