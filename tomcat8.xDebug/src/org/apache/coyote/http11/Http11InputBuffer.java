...
public class Http11InputBuffer implements InputBuffer {
    ...
    void init(SocketWrapperBase<?> socketWrapper) {
        ...
        wrapper = socketWrapper;//!!!!! 注入socket
    ...
    private HeaderParseStatus parseHeader()
        ...
        while (headerParsePos == HeaderParsePosition.HEADER_NAME) {
            ...
            if (chr == Constants.COLON) {
                ...
                headerData.headerValue = headers.addValue(buf, headerData.start, pos - headerData.start); // 创建MimeHeaderField对象，并添加到headers
                pos++;
                // Mark the current buffer position
                headerData.start = pos;
        ...
        headerData.headerValue.setBytes(buf, headerData.start,
                headerData.lastSignificantChar - headerData.start); // 设置头的值
        headerData.recycle(); // 回收掉headerValue
        return HeaderParseStatus.HAVE_MORE_HEADERS; // 解析下一个头
    }
