...
public class TagPluginManager {
    ...
    public TagPluginManager(ServletContext ctxt) {
        this.ctxt = ctxt; // org.apache.catalina.core.ApplicationContextFacade
    }
...
//    	<tag-plugins>
//    		<tag-class>cn.java.tagplugin.TagClass1</tag-class>
//    		<plugin-class>cn.java.tagplugin.PluginClass1</plugin-class>
//    	<tag-plugins>
        init(err);  // 初始化
        
//        TagPluginManager = {
//            tagPlugins ： {
//            	"cn.java.tagplugin.TagClass1" : new cn.java.jsp.tagplugin.PluginClass1();
//            	"cn.java.tagplugin.TagClass2" : new cn.java.jsp.tagplugin.PluginClass2();
//            } 	
//        }
    ...
    private void init(ErrorDispatcher err) throws JasperException {
        ...
        try {
            ...
            Enumeration<URL> urls =
                    ctxt.getClassLoader().getResources(META_INF_JASPER_TAG_PLUGINS_XML); // META-INF/org.apache.jasper/tagPlugins.xml
            while (urls.hasMoreElements()) {
                URL url = urls.nextElement();
                parser.parse(url);
            }

            URL url = ctxt.getResource(TAG_PLUGINS_XML); // /WEB-INF/tagPlugins.xml
            if (url != null) {
//            	<tag-plugins>
//	        		<tag-class>cn.java.tagplugin.TagClass1</tag-class>
//	        		<plugin-class>cn.java.tagplugin.PluginClass1</plugin-class>
//	        	<tag-plugins>
                parser.parse(url); // 解析插件成对象树
            }
        ...
        Map<String, String> plugins = parser.getPlugins();  // 取得插件列表
        tagPlugins = new HashMap<>(plugins.size());
        for (Map.Entry<String, String> entry : plugins.entrySet()) {
            try {
                String tagClass = entry.getKey(); // cn.java.tagplugin.TagClass1
                String pluginName = entry.getValue(); // cn.java.jsp.tagplugin.PluginClass1
                Class<?> pluginClass = ctxt.getClassLoader().loadClass(pluginName);
                TagPlugin plugin = (TagPlugin) pluginClass.newInstance(); // 实例化标签插件
                tagPlugins.put(tagClass, plugin); // 存储“标签->标签插件”映射
...
//       	"cn.java.tagplugin.TagClass2" : new cn.java.jsp.tagplugin.PluginClass2();
//       }    
        TagPlugin tagPlugin = tagPlugins.get(n.getTagHandlerClass().getName()); // 标签对应插件
        if (tagPlugin == null) {
            return;
        }
        ...
        tagPlugin.doTag(tagPluginContext);  // 调用插件的 doTag
    }
