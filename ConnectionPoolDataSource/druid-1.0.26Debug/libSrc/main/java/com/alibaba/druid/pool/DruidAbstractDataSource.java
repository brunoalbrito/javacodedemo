...
public abstract class DruidAbstractDataSource extends WrapperAdapter implements DruidAbstractDataSourceMBean, DataSource, DataSourceProxy, Serializable {
    ...
    public void validateConnection(Connection conn) throws SQLException {
        ...
        if (validConnectionChecker != null) { // 连接检查器 com.alibaba.druid.pool.vendor.MySqlValidConnectionChecker
            boolean result = true;
    ...
    public PhysicalConnectionInfo createPhysicalConnection() throws SQLException {
        ...
        if (user != null && user.length() != 0) {
            physicalConnectProperties.put("user", user); // 用户名
        }

        if (password != null && password.length() != 0) {
            physicalConnectProperties.put("password", password); // 密码
        }
        ...
        try {
            conn = createPhysicalConnection(url, physicalConnectProperties); // 创建连接 ，如果有配置过滤器，返回的是代理对象com.alibaba.druid.proxy.jdbc.ConnectionProxyImpl
            ...
            initPhysicalConnection(conn); // 初始化连接
            initedNanos = System.nanoTime();

            validateConnection(conn); // 检查连接是否正常
    ...
    public void initPhysicalConnection(Connection conn) throws SQLException {
        ...
        try {
            stmt = conn.createStatement();
            for (String sql : initSqls) { // 初始执行的SQL
                if (sql == null) {
                    continue;
                }
