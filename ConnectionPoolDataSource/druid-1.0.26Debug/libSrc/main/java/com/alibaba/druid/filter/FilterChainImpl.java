...
public class FilterChainImpl implements FilterChain {
    ...
    public void connection_setAutoCommit(ConnectionProxy connection, boolean autoCommit) throws SQLException {
        ...
        connection.getRawObject().setAutoCommit(autoCommit); // 走完过滤器链条，执行实际对象的方法
    }
    ...
    public boolean preparedStatement_execute(PreparedStatementProxy statement) throws SQLException {
        if (this.pos < filterSize) {
            return nextFilter().preparedStatement_execute(this, statement); // 执行过滤器链
        }
        return statement.getRawObject().execute(); // 执行实际jdbc对象
    }
    ...
    public DruidPooledConnection dataSource_connect(DruidDataSource dataSource, long maxWaitMillis) throws SQLException {
        if (this.pos < filterSize) {
            DruidPooledConnection conn = nextFilter().dataSource_getConnection(this, dataSource, maxWaitMillis); // 执行过滤器链条
            return conn;
        }
