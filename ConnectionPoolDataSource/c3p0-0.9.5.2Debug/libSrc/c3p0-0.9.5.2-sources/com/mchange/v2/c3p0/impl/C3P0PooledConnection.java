...
public final class C3P0PooledConnection extends AbstractC3P0PooledConnection
{
	...
	{ 
		...
		this.physicalConnection = con; // 物理连接
	...
	private synchronized void close(boolean known_invalid) throws SQLException
	{
		...
		if ( physicalConnection != null )
		{
			...
			{ 
				...
				try { physicalConnection.close(); } // 关闭物理连接
				catch (Exception e)
				{
					if (Debug.DEBUG)
					{
			...
			if ( activeConnection != null )
			{
				...
				{ 
					if (! pooled_connection_is_dead)
						C3P0PooledConnection.this.reset(known_resolved_txn); // !!!! 重置
				}
				catch (Exception e)
				{ 
					exc1 = e;
				...
				exc2 = cleanupUncachedActiveStatements(); // 清除正在执行的语句
				...
				if (!closeAndRemoveResultSets( activeMetaDataResultSets )) // 关闭结果集
