...
public final class C3P0PooledConnectionPoolManager
{
	...
	private void poolsInit()
	{
		/*
		 	...
		 	{
			 	!!! privilegeSpawnedThreads = C3P0Config.initializeBooleanPropertyVar("privilegeSpawnedThreads", C3P0Defaults.privilegeSpawnedThreads());  === false
			 	!!! contextClassLoaderSource = C3P0Config.initializeStringPropertyVar("contextClassLoaderSource", C3P0Defaults.contextClassLoaderSource()); === "caller"
		 	}
	...
	private synchronized void _poolsInit()
	{
		...
		// statementCacheNumDeferredCloseThreads = C3P0Config.initializeIntPropertyVar("statementCacheNumDeferredCloseThreads", C3P0Defaults.statementCacheNumDeferredCloseThreads()); === 0
					...
					throws SQLException
	{
		try
		{
			...
			Map tmp = new HashMap();
			BeanInfo bi = Introspector.getBeanInfo( cpds.getClass() ); // com.mchange.v2.c3p0.WrapperConnectionPoolDataSource
			PropertyDescriptor[] pds = bi.getPropertyDescriptors(); // 获取所有属性
