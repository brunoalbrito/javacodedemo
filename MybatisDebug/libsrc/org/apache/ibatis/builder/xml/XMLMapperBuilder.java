...
public class XMLMapperBuilder extends BaseBuilder {
    ...
    /*
	    <mapper namespace="namespace1"> <!-- builderAssistant.setCurrentNamespace("namespace1"); -->
	    	...
	    	<cache type="PERPETUAL" eviction="LRU" flushInterval="" size="" readOnly="false" blocking="false"  > <!-- builderAssistant.useNewCache(typeClass, evictionClass, flushInterval, size, readWrite, blocking, props); -->
		 	...
		 	<!-- resultMap -->
		 	<resultMap id="" type="类型优先级1" ofType="类型优先级2" resultType="类型优先级3" javaType="类型优先级4" extends="" autoMapping=""> <!-- configuration.addResultMap(resultMap); builderAssistant.addResultMap(this.id, this.type, this.extend, this.discriminator, this.resultMappings, this.autoMapping); -->
		  		<constructor>
  ...
  private void cacheRefElement(XNode context) {
    if (context != null) {
      ...
      try {
        cacheRefResolver.resolveCacheRef(); // builderAssistant.useCacheRef(context.getStringAttribute("namespace"));
      } catch (IncompleteElementException e) {
  ...
  private void cacheElement(XNode context) throws Exception {
	/*
	 	<cache type="PERPETUAL" eviction="LRU" flushInterval="" size="" readOnly="false" blocking="false"  >
	 		<property name="" value="" />
	 		<property name="" value="" />
	 	</cache>
	 */
    if (context != null) {
      ...
      Class<? extends Cache> typeClass = typeAliasRegistry.resolveAlias(type); // org.apache.ibatis.cache.impl.PerpetualCache
      String eviction = context.getStringAttribute("eviction", "LRU"); 
      Class<? extends Cache> evictionClass = typeAliasRegistry.resolveAlias(eviction); // org.apache.ibatis.cache.decorators.LruCache
...
//	  	  </discriminator>
  ...
  private void bindMapperForNamespace() {
    ...
    if (namespace != null) {
      ...
      try {
        boundType = Resources.classForName(namespace); // 加载XxxMapper.xml中命名空间中填写的接口“com.test.bean.XxxMapper”
      } catch (ClassNotFoundException e) {
        //ignore, bound type is not required
      }
      if (boundType != null) { // 如果是以
