...
public class ResolverUtil<T> {
  ...
  public ResolverUtil<T> find(Test test, String packageName) {
    String path = getPackagePath(packageName); // path = "cn/java/package1"

    try {
      List<String> children = VFS.getInstance().list(path); // 查找指定包（包括子目录）路径下的类 org.apache.ibatis.io.DefaultVFS.list(path);
