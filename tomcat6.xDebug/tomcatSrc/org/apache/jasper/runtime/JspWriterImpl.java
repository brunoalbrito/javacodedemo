...
public class JspWriterImpl extends JspWriter {
    ...
    public JspWriterImpl(ServletResponse response, int sz, 
        ...
        this.response = response;// ServletResponseWrapperInclude
...
//        	response == ServletResponseWrapperInclude
            out = response.getWriter();//输出对象   out =  response.getWriter();
        }
    }
    ...
    public void write(String s, int off, int len) throws IOException {
        ensureOpen();
        if (bufferSize == 0) {
            initOut();//!!!!!!!!!!初始化    out =  response.getWriter();
            out.write(s, off, len);
            return;
        }
        int b = off, t = off + len;
