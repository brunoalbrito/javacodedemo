...
public class JspServletWrapper {
        ...
        if (reload) {
            synchronized (this) {
                ...
                if (reload) {
                    ...
                    try {
                        servletClass = ctxt.load();//加载类
                        servlet = (Servlet) servletClass.newInstance();//创建Servlet对象!!!!!!!!!!!!
                        //注解处理器
                    ...
                    servlet.init(config);//执行Servlet的init方法  org.apache.jasper.runtime.HttpJspBase.init(config); 
    ...
    public void service(HttpServletRequest request, 
        ...
        try {
            ...
            if (ctxt.isRemoved()) {
                throw new FileNotFoundException(jspUri);//文件不存在
            }
             ...
             * 编译JSP文件成Servlet类文件
             */
            if (options.getDevelopment() || firstTime ) {//开发模式，或者首次执行!!!!!!!!!!!!!!!
                synchronized (this) {
                    firstTime = false;

                    // The following sets reload to true, if necessary
                    ctxt.compile();//  编译JSP文件  org.apache.jasper.JspCompilationContext.compile() 
                }
            } else {
                if (compileException != null) {
        ...
        try {
            ...
            if (servlet instanceof SingleThreadModel) {
               ...
               synchronized (this) {
            	...
            	/**
            	 * 执行Servlet类对象!!!!!!!!!!!!!!
            	 * org.apache.jasper.runtime.HttpJspBase.service(request, response);
            	 */
                servlet.service(request, response);//执行jsp的Servlet对象，进行输出
            }
