...
public class XMLEncodingDetector {
        ...
        throws IOException, JasperException
    {
        ...
        Object[] ret = detector.getEncoding(inStream, err);//取得编码
        inStream.close();
	...
	if (encoding == null) {
	    ...
	    if (count == 4) {//读取四个字节
		Object [] encodingDesc = getEncodingName(b4, count);//取得编码名称!!!!!
		encoding = (String)(encodingDesc[0]);//编码名称
    ...
    private void scanXMLDeclOrTextDecl(boolean scanningTextDecl,
        ...
        while (peekChar() != '?') {
            ...
            switch (state) {
                case STATE_VERSION: {
                    if (name == fVersionSymbol) { // < version
                        if (!sawSpace) {
			    ...
			    // in such cases.
                    ...
                    } else if (name == fEncodingSymbol) {// < encoding >
                        if (!scanningTextDecl) {
                ...
                case STATE_ENCODING: {
                    if (name == fEncodingSymbol) { // < encoding >
                        if (!sawSpace) {
                            reportFatalError(scanningTextDecl
                                      ? "jsp.error.xml.spaceRequiredBeforeEncodingInTextDecl"
                                      : "jsp.error.xml.spaceRequiredBeforeEncodingInXMLDecl",
                        ...
                        //       entity scanner
                    } else if (!scanningTextDecl && name == fStandaloneSymbol) { //  <standalone>
                        if (!sawSpace) {
                ...
                case STATE_STANDALONE: {
                    if (name == fStandaloneSymbol) {//  < standalone >
                        if (!sawSpace) {
                            err.jspError("jsp.error.xml.spaceRequiredBeforeStandalone");
                        }
                        standalone = fString.toString();
