...
public class JspConfig {
    ...
    private void processWebDotXml(ServletContext ctxt) throws JasperException {
        ...
        try {
//        	ctxt === org.apache.catalina.core.ApplicationContextFacade
            URL uri = ctxt.getResource(WEB_XML);// 读取文件：/WEB-INF/web.xml
            if (uri == null) {
                // no web.xml
                return;
            }
            ...
            ParserUtils pu = new ParserUtils(validate, blockExternal);//解析器!!!!!!!
            TreeNode webApp = pu.parseXMLDocument(WEB_XML, ip);//!!!!!!!!!! 解析/WEB-INF/web.xml

            if (webApp == null
                    || getVersion(webApp) < 2.4) {//应用的版本小于 2.4
            ...
            while (jspPropertyList.hasNext()) {
                ...
                TreeNode element = (TreeNode) jspPropertyList.next();//jspPropertyList == <jsp-property-group>
                ...
                while (list.hasNext()) {// <jsp-property-group>的子节点
                ...
                for( int p = 0; p < urlPatterns.size(); p++ ) {
                    ...
                    if (urlPattern.indexOf('*') < 0) { //  *
                        ...
                        int i = urlPattern.lastIndexOf('/');//  /dir1/dir2/*
                        String file;
                        if (i >= 0) {
                            path = urlPattern.substring(0,i+1); // /dir1/dir2/
                        ...
                        if (file.equals("*")) { //  /dir1/dir2/*
                            extension = "*";
                        } else if (file.startsWith("*.")) {//  /dir1/dir2/*.jsp
    ...
    private void init() throws JasperException {
        ...
        if (!initialized) {
            synchronized (this) {
                if (!initialized) {
                    processWebDotXml(ctxt);// 处理 /WEB-INF/web.xml 
    ...
    public JspProperty findJspProperty(String uri) throws JasperException {
        ...
        init();//!!!!!!!!!!!!  在/WEB-INF/web.xml文件中读取JSP节点<jsp-config>的配置
        ...
        if (index >=0 ) {
            uriPath = uri.substring(0, index+1);//uri路径
        }
        String uriExtension = null;
        index = uri.lastIndexOf('.');
        if (index >=0) {
            uriExtension = uri.substring(index+1);//文件扩展名
        }
        ...
        while (iter.hasNext()) {
            ...
            String extension = jpg.getExtension();//扩展名
            String path = jpg.getPath();//路径
            ...
            if (jp.getIncludePrelude() != null) {
                includePreludes.addAll(jp.getIncludePrelude());//要include的
            }
            if (jp.getIncludeCoda() != null) {
                includeCodas.addAll(jp.getIncludeCoda());//要尾部引入的
            }
            ...
            if (jp.isELIgnored() != null) {
                elIgnoredMatch = selectProperty(elIgnoredMatch, jpg);//EL表达式
            }
            ...
            if (jp.getPageEncoding() != null) {
                pageEncodingMatch = selectProperty(pageEncodingMatch, jpg);//页面编码
            }
            ...
            if (jp.isTrimDirectiveWhitespaces() != null) {
                trimDirectiveWhitespacesMatch =
                    selectProperty(trimDirectiveWhitespacesMatch, jpg);//去除空格
            }
        }
