...
class JspReader {
    ...
    public JspReader(JspCompilationContext ctxt,
                     String fname,
                     String encoding,
                     InputStreamReader reader,//!!!!!! /index.jsp 文件的输入流
                     ErrorDispatcher err)
    ...
    private void pushFile(String file, String encoding, 
        ...
        // Register the file
        String longName = file;//  file = "/index.jsp" 文件的输入流

        int fileid = registerSourceFile(longName);//  文件栈的索引ID

        if (fileid == -1) { // 重复解析jsp文件
            // Bugzilla 37407: http://issues.apache.org/bugzilla/show_bug.cgi?id=37407
            if(reader != null) {//  /index.jsp 文件的输入流
                try {
                    reader.close();// 关闭
                } catch (Exception any) {
