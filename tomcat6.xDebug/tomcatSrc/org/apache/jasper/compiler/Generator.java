...
class Generator {
    ...
    private void genPreambleStaticInitializers() throws JasperException {
    	/**
    	 	...
    	 	static {
    	 		_jspx_dependants = new java.util.ArrayList(size);
    	 		_jspx_dependants.add("1");
    	 		_jspx_dependants.add("2");
    	 		_jspx_dependants.add("3");
    	 	}
    	 */
    ...
    private void genPreambleClassVariableDeclarations(String className)
        ...
        out.print(VAR_EXPRESSIONFACTORY);//_el_expressionfactory
        out.println(";");
        out.printin("private org.apache.AnnotationProcessor ");
        out.print(VAR_ANNOTATIONPROCESSOR);// _jsp_annotationprocessor
        out.println(";");
        out.println();
    }
    ...
    private void generatePreamble(Node.Nodes page) throws JasperException {
        ...
        // First the package name:  声明包名
        genPreamblePackage(servletPackageName);  //package  cn.java.jspservlet

        // Generate imports 声明导入包
        genPreambleImports();  // import cn.java.Clazz1; import cn.java.Clazz2;
        ...
        out.printin("public final class ");// 声明Servlet类
        out.print(servletClassName); // index_jsp
        out.print(" extends ");
        out.println(pageInfo.getExtends()); // org.apache.jasper.runtime.HttpJspBase
        ...
        */
        ...
        declareTemporaryScriptingVars(page);// 渲染也中的声明临时变量节点Nodes
        out.println();
    ...
    class GenerateVisitor extends Node.Visitor {
         ...
         * 表达式
         * 
         * <%=request.getContextPath()%> 等价 out.print(request.getContextPath());
         * <jsp:expression>request.getContextPath()</jsp:expression> 等价 out.print(request.getContextPath());
         */
        ...
        public void visit(Node.ELExpression n) throws JasperException {
            ...
            if (!pageInfo.isELIgnored() && (n.getEL() != null)) {
            	//out.write((java.lang.String) org.apache.jasper.runtime.PageContextImpl.proprietaryEvaluate("${param.attrName1}", java.lang.String.class, (PageContext)_jspx_page_context, null, false));
    ...
    public static void generate(ServletWriter out, Compiler compiler,
        ...
        if (gen.ctxt.isTagFile()) {
            ...
            gen.generatePreamble(page);// !!!!!!!!!  把JAVA 代码写入 *.java 文件   try{ 开头的代码   ....   "_jspx_out = out;"
            gen.generateXmlProlog(page);//  !!!!!   把输出HTML依赖库的代码写入 *.java 文件
            gen.fragmentHelperClass.generatePreamble(); //  private class  MyClassName extends org.apache.jasper.runtime.JspFragmentHelper
            /**
               		迭代调用JSP文件解析处理的Nodes列表，迭代调用每个Node的accept方法
               		生成JAVA代码
             */
            page.visit(gen.new GenerateVisitor(gen.ctxt.isTagFile(), out,
                    gen.methodsBuffered, gen.fragmentHelperClass, gen.ctxt
                            .getClassLoader(), null));
            gen.generatePostamble(page); //try 结尾的代码
        }
    }
    ...
    private static class FragmentHelperClass {
        ...
        public void generatePreamble() {
            ...
            out.pushIndent();
            /**
            	private class  MyClassName extends org.apache.jasper.runtime.JspFragmentHelper{
            		private javax.servlet.jsp.tagext.JspTag _jspx_parent;
            		private int[] _jspx_push_body_count;
            		public MyClassName( int discriminator, JspContext jspContext,
