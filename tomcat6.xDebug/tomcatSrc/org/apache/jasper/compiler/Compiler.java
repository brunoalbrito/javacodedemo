...
public abstract class Compiler {
    ...
    public void init(JspCompilationContext ctxt, JspServletWrapper jsw) {
        this.jsw = jsw;
        this.ctxt = ctxt;
        this.options = ctxt.getOptions();  // org.apache.jasper.EmbeddedServletOptions
    }
    ...
    protected String[] generateJava() throws Exception {
        ...
        // Setup page info area 页面信息
        pageInfo = new PageInfo(new BeanRepository(ctxt.getClassLoader(),
                errDispatcher), ctxt.getJspFile());//jsp页面信息

        JspConfig jspConfig = options.getJspConfig();//<servlet>jsp</servlet>的配置信息   org.apache.jasper.EmbeddedServletOptions.getJspConfig();
        JspConfig.JspProperty jspProperty = jspConfig.findJspProperty(ctxt
                .getJspFile());//jsp的属性!!!!!! 在/WEB-INF/web.xml文件中读取JSP节点<jsp-config>的配置
         ...
         * properties.
         */
        if (jspProperty.isELIgnored() != null) {//忽略EL表达式
            pageInfo.setELIgnored(JspUtil.booleanValue(jspProperty
                    .isELIgnored()));//默认值是false
        }
        ...
        ctxt.checkOutputDir();
        String javaFileName = ctxt.getServletJavaFileName();//Servlet的 java文件名称
        ...
        try {
             ...
             */ 
            // Parse the file  解析Jsp文件!!!!!!!!!!
            ParserController parserCtl = new ParserController(ctxt, this);
            
            //System.out.println("ctxt.getJspFile()---"+ctxt.getJspFile());//  "/index.jsp"
            //"E:\Repository\EclipseRepository\tomcatDebug\work\Catalina\localhost\_\org\apache\jsp\index_jsp.java
            //System.out.println("ctxt.getServletJavaFileName()---"+javaFileName); 
            
            /**
	             <jsp:directive.page import="org.apache.jasper.JspCompilationContext" />
	             <%@ page import="org.apache.jasper.JspCompilationContext" %>
             */
            // Pass 1 - the directives  解析出所有指令节点
            Node.Nodes directives =
                parserCtl.parseDirectives(ctxt.getJspFile());//!!!!!!!!!!!!!!
            Validator.validateDirectives(this, directives);// 检查是否有<%@ page import="..." %>节点，注入到pageInfo , pageInfo.addImports(.....);

            /**
             	!!!!!!!!!!!!!!!!!!!!!!!!
             		感知JSP文件的编码，读取JSP文件，解析成pageNodes的形式
             	!!!!!!!!!!!!!!!!!!!!!!!!
             */
            // Pass 2 - the whole translation unit  org.apache.jasper.compiler.Node.Nodes
            pageNodes = parserCtl.parse(ctxt.getJspFile()); // 解析模板标签的各个节点 
            
            if (ctxt.isPrototypeMode()) {
            	System.out.println("走这里吗？ NO ");
            ...
            // Collect page info 收集页面的信息
            Collector.collect(this, pageNodes);

            // Compile (if necessary) and load the tag files referenced in
            // this compilation unit.
            tfp = new TagFileProcessor(); // 标签文件处理器
            tfp.loadTagFiles(this, pageNodes); // 加载自定义标签处理器
            ...
            TextOptimizer.concatenate(this, pageNodes);//文本优化器

            // Generate static function mapper codes.
            ELFunctionMapper.map(this, pageNodes);// el表达式，函数映射器

            // generate servlet .java file    生成 .JAVA文件!!!!!!!!!
            writer = setupContextWriter(javaFileName);
            //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            //!!!!!!!!!!!!!!!!!!!!把JSP页面中的代码解析成NODE类型的数据，生成JAVA文件!!!!!!!!!!!!!!!!!!!!!!!
            //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            Generator.generate(writer, this, pageNodes);//通过解析jsp文件，解析成pageNodes的形式，保存成JAVA文件
            writer.close();
            writer = null;
    ...
    public void compile(boolean compileClass, boolean jspcMode)
        ...
        try {
        	/**
            	...
            	!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        	 */
        	System.out.println("org.apache.jasper.compiler.Compiler.compile(...)---生成java文件");
            String[] smap = generateJava();//产生Java文件!!!!!!!!!!!
            if (compileClass) {//true
            	/**
	        	  	org.apache.jasper.compiler.JDTCompiler.generateClass(...)
	        	  	!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	            	!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	            	!!!!!!!!!!!!!!!!!!!!!把JAVA文件的代码，转换成class文件!!!!!!!!!!!!!!!!!!!!!!
	            	!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	            	!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	        	 */
            	System.out.println("org.apache.jasper.compiler.Compiler.compile(...)---编译java文件");
                generateClass(smap);//编译成  class文件!!!!!!!!!!!!!!
    ...
    public boolean isOutDated(boolean checkClass) {
        ...
        System.out.println("org.apache.jasper.compiler.Compiler.isOutDated(..)--"+jsp);
        if (jsw != null
                && (ctxt.getOptions().getModificationTestInterval() > 0)) {//定时检查周期
        ...
        if (checkClass) {
            targetFile = new File(ctxt.getClassFileName());//检查Class文件
        } else {
    ...
    public void removeGeneratedFiles() {
        removeGeneratedClassFiles();// !!!!!! 删除生成的CLASS文件

        try {
            String javaFileName = ctxt.getServletJavaFileName();//
            //E:\Repository\EclipseRepository\tomcatDebug\work\Catalina\localhost\_\org\apache\jsp\index_jsp.java
            System.out.println("org.apache.jasper.compiler.Compiler.removeGeneratedFiles()--删除java文件--"+javaFileName);
            if (javaFileName != null) {
                File javaFile = new File(javaFileName);
                if (log.isDebugEnabled())
                    log.debug("Deleting " + javaFile);
                if (javaFile.exists()) {
                    if (!javaFile.delete()) {//删除生成的java文件
                        log.warn(Localizer.getMessage(
                                "jsp.warning.compiler.javafile.delete.fail",
    ...
    public void removeGeneratedClassFiles() {
        try {
            ...
            System.out.println("org.apache.jasper.compiler.Compiler.removeGeneratedClassFiles()--删除class文件--"+classFileName);
