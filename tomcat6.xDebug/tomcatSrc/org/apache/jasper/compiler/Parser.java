...
class Parser implements TagConstants {
    ...
    public static Node.Nodes parse(ParserController pc, JspReader reader,
        ...
        Node.Root root = new Node.Root(reader.mark(), parent, false);//根节点
        root.setPageEncoding(pageEnc);//页面编码
        ...
        PageInfo pageInfo = pc.getCompiler().getPageInfo();//org.apache.jasper.compiler.JDTCompiler.getPageInfo();
        if (parent == null && !isTagFile) {
        	// 添加include的文件
            parser.addInclude(root, pageInfo.getIncludePrelude());//<jsp-config>定义的头部的 <include-prelude>/WEB-INF/view/common/header.jsp</include-prelude>
        }
        if (directivesOnly) {
            ...
            while (reader.hasMoreInput()) {
                 ...
                 */
                parser.parseElements(root);// 解析其他内容的节点 !!!!!!!!!!!!
            }
        }
        if (parent == null && !isTagFile) {
            parser.addInclude(root, pageInfo.getIncludeCoda());// <jsp-config>定义的尾部的<include-coda>/WEB-INF/view/common/footer.jsp</include-coda>
        }
    ...
    private boolean parseAttribute(AttributesImpl attrs) throws JasperException {
    	...
    	// 如：  <%@ page import="cn.java.HelloWorld" %>
        // Get the qualified name
        String qName = parseName(); //   解析键
        if (qName == null)
            return false;
        ...
        if (index != -1) {
            String prefix = qName.substring(0, index);//  prefix:localName
        ...
        if (reader.matches("<%="))
            watchString = "%>";
        watchString = watchString + quote;//  import="cn.java.HelloWorld"  import="<%=cn.java.HelloWorld%>"

        String attrValue = parseAttributeValue(watchString);//  解析值      %>"  或者  "
    ...
    private String parseName() throws JasperException {
        ...
        if (Character.isLetter(ch) || ch == '_' || ch == ':') {//如果是字母 或者 以_开头 或者 以:开头
            StringBuffer buf = new StringBuffer();
            buf.append(ch);
            reader.nextChar();//移动到下一字符
            ch = (char) reader.peekChar();//读取字符
    ...
    private String parseAttributeValue(String watch) throws JasperException {
        ...
        String ret = null;
        try {
            char quote = watch.charAt(watch.length() - 1); //结束标记，如  "
    ...
    private void processIncludeDirective(String file, Node parent)
        ...
        try {
            parserController.parse(file, parent, jarFileUrl);//解析文件
    ...
    private void parsePageDirective(Node parent) throws JasperException {
    	//  <%@ page import="cn.java.HelloWorld" %> 
        Attributes attrs = parseAttributes();// 解析属性
    ...
    private void addInclude(Node parent, List files) throws JasperException {
        if (files != null) {
            ...
            while (iter.hasNext()) {
                ...
                AttributesImpl attrs = new AttributesImpl();
                attrs.addAttribute("", "file", "file", "CDATA", file);//定义属性
                // <jsp:directive.include file="" />
                // Create a dummy Include directive node
                Node includeNode = new Node.IncludeDirective(attrs, reader
                        .mark(), parent);
                processIncludeDirective(file, includeNode);// 解析节点
            }
        }
    }
    ...
    private void parseTaglibDirective(Node parent) throws JasperException {
        ...
        /**
         	web.xml中的配置
	         <taglib>
				<taglib-location>/WEB-INF/jsp/mytags-taglib.tld</taglib-location>
				<taglib-uri>http://cn.java.servlet/mytags-taglib</taglib-uri>
			 </taglib>
			----------------------------------
			JSP文件头部的声明
			<%@ taglib uri="cx" prefix="http://java.sun.com/jsp/jstl/core" %>
         */
        String uri = attrs.getValue("uri"); // 如： uri="http://java.sun.com/jsp/jstl/core" 
        String prefix = attrs.getValue("prefix");//  如：prefix="cx" 
        if (prefix != null) {// !!!!!!
            ...
            if (uri != null) {// 如： uri="http://java.sun.com/jsp/jstl/core" 
                String uriPrev = pageInfo.getURI(prefix);//  如：prefix="cx" 
                if (uriPrev != null && !uriPrev.equals(uri)) {
                    err.jspError(reader.mark(), "jsp.error.prefix.refined",
                            prefix, uri, uriPrev);
                }
                if (pageInfo.getTaglib(uri) == null) { // 取得标签库的信息   uri =  http://cn.java.servlet/mytags-taglib
                    TagLibraryInfoImpl impl = null;
                    if (ctxt.getOptions().isCaching()) { // 有缓存
                        impl = (TagLibraryInfoImpl) ctxt.getOptions()
                                .getCache().get(uri);
                    }
                    if (impl == null) {
                    	// mappings.put("http://cn.java.servlet/mytags-taglib", new String[] { "/WEB-INF/jsp/mytags-taglib.tld", null });
                        String[] location = ctxt.getTldLocation(uri); //org.apache.jasper.JspCompilationContext.getTldLocation(uri);
                    ...
                    pageInfo.addTaglib(uri, impl);//添加URI地址，对应的标签库信息
                }
                pageInfo.addPrefixMapping(prefix, uri);//添加前缀对应的URI地址
            } else {
            	//<%@ taglib uri="cx" prefix="http://java.sun.com/jsp/jstl/core" tagdir="..." %>
                String tagdir = attrs.getValue("tagdir");
                if (tagdir != null) {
                    String urnTagdir = URN_JSPTAGDIR + tagdir; //urn:jsptagdir:
    ...
    private void parseDirective(Node parent) throws JasperException {
        ...
        if (reader.matches("page")) {
            ...
            //parentBody.add("org.apache.jasper.compiler.Node.PageDirective");
            parsePageDirective(parent);
        } else if (reader.matches("include")) {
            directive = "&lt;%@ include";
            //parentBody.add("org.apache.jasper.compiler.Node.IncludeDirective");
            ...
            directive = "&lt;%@ taglib";
            //<%@ taglib uri="cx" prefix="http://java.sun.com/jsp/jstl/core" %>
            //parentBody.add("org.apache.jasper.compiler.Node.TaglibDirective");
            parseTaglibDirective(parent);// 解析标签指令!!!!!!!!!!
            ...
            //parentBody.add("org.apache.jasper.compiler.Node.TagDirective");
            ...
            //parentBody.add("org.apache.jasper.compiler.Node.AttributeDirective");
            ...
            //parentBody.add("org.apache.jasper.compiler.Node.VariableDirective");
...
//        new Node.ELExpression('#', reader.getText(start, last), start, parent);
    ...
    private void parseOptionalBody(Node parent, String tag, String bodyType)
        ...
        if (!parseJspAttributeAndBody(parent, tag, bodyType)) { // bodyType == "JSP"
            // Must be ( '>' # Body ETag )
    ...
    private boolean parseCustomTag(Node parent) throws JasperException {
        ...
        String tagName = reader.parseToken(false); // <mtag:echo ... />
        ...
        String prefix = tagName.substring(0, i); //mtag
        String shortTagName = tagName.substring(i + 1);  //echo

        // Check if this is a user-defined tag.
        String uri = pageInfo.getURI(prefix);//  "http://cn.java.servlet/mytags-taglib"
        if (uri == null) {
            reader.reset(start);
        ...
        TagLibraryInfo tagLibInfo = pageInfo.getTaglib(uri); // 取得标签库信息   tagLibInfo === org.apache.jasper.compiler.TagLibraryInfoImpl
        TagInfo tagInfo = tagLibInfo.getTag(shortTagName); // 标签信息  org.apache.jasper.compiler.TagLibraryInfoImpl.getTag("echo");
        TagFileInfo tagFileInfo = tagLibInfo.getTagFile(shortTagName); //为null， 标签文件信息  org.apache.jasper.compiler.TagLibraryInfoImpl.getTagFile("echo");
        ...
        if (tagInfo != null) {
        	/**
	        	 <tag>
						<name>echo</name>
						<tag-class>cn.java.servlet.tags.Echo</tag-class>
            ...
            String handlerClassName = tagInfo.getTagClassName(); //取得标签处理类名称 cn.java.servlet.tags.Echo
            try {
                tagHandlerClass = ctxt.getClassLoader().loadClass(
                        handlerClassName); // 加载处理类
            } catch (Exception e) {
        ...
        Attributes attrs = parseAttributes(); //  <mtag:echo attr1="value1"  attr2="value2"... />
        reader.skipSpaces();

        // Parse 'CustomActionEnd' production:
        if (reader.matches("/>")) {
            if (tagInfo != null) {//!!!!
            	//CustomTag("mtag:echo","mtag","echo","http://cn.java.servlet/mytags",attrs,start, parent, tagInfo标签信息 , '标签处理类')
        ...
        String bc;
        if (tagInfo != null) {
            bc = tagInfo.getBodyContent();//!!!! bc = "JSP"
        } else {
    ...
    private void parseElements(Node parent) throws JasperException {
        ...
        } else if (reader.matches("<jsp:directive.")) {//3
            parseXMLDirective(parent);
        } else if (reader.matches("<%!")) {//4  定义方法
            parseDeclaration(parent);
        } else if (reader.matches("<jsp:declaration")) {//5
            parseXMLDeclaration(parent);
        } else if (reader.matches("<%=")) {//6  <%=  out.print();
            parseExpression(parent);
        } else if (reader.matches("<jsp:expression")) {//7
            parseXMLExpression(parent);
        } else if (reader.matches("<%")) {//8  java 代码
            parseScriptlet(parent);
        } else if (reader.matches("<jsp:scriptlet")) {//9  
            parseXMLScriptlet(parent);
        } else if (reader.matches("<jsp:text")) {//10
            parseXMLTemplateText(parent);
        } else if (!pageInfo.isELIgnored() && reader.matches("${")) {//11
            parseELExpression(parent, '$');
        } else if (!pageInfo.isELIgnored()
                && !pageInfo.isDeferredSyntaxAllowedAsLiteral()
                && reader.matches("#{")) {//12
            parseELExpression(parent, '#');
        } else if (reader.matches("<jsp:")) {//13  标准的标签
            parseStandardAction(parent);
        } else if (!parseCustomTag(parent)) {//14  解析自定义标签  <mtag:echo att1="" /> 或者 普通文本
            checkUnbalancedEndTag();
            parseTemplateText(parent);//  解析HTML文本
        }
    }
