...
class ParserController implements TagConstants {
     ...
     */
    private Node.Nodes doParse(String inFileName,//jsp文件
            Node parent,//null
            URL jarFileUrl)//jar文件地址
    throws FileNotFoundException, JasperException, IOException {
        ...
        String absFileName = resolveFileName(inFileName);//取得JSP文件的绝对路径 "/index.jsp"
        String jspConfigPageEnc = getJspConfigPageEncoding(absFileName);//取得文件的编码类型，根<jsp-config>的编码声明 <page-encoding>

//        头部声明  <%@ page language="java" contentType="text/html; charset=UTF-8"  pageEncoding="UTF-8"%>
        // Figure out what type of JSP document and encoding type we are
        // dealing with
        determineSyntaxAndEncoding(absFileName, jarFile, jspConfigPageEnc);//检查语法和编码
        System.out.println("-----doParse---start-----");
        System.out.println("org.apache.jasper.compiler.ParserController.doParse(..)--absFileName="+absFileName);
        System.out.println("org.apache.jasper.compiler.ParserController.doParse(..)--sourceEnc="+sourceEnc);
        ...
        if ((isXml && isEncodingSpecifiedInProlog) || isBomPresent) {//false
            /*
        ...
        if (isXml) {//false
        	...
        	System.out.println("读取文件流--解析出节点 Node.Nodes YES");
            // Standard syntax
            ...
            try {
                ...
                //创建 org.apache.jasper.compiler.Mark
    ...
    throws JasperException {
        ...
        JspConfig jspConfig = ctxt.getOptions().getJspConfig();// <servlet>jsp</servlet>节点的配置信息
        JspConfig.JspProperty jspProperty
            = jspConfig.findJspProperty(absFileName);// <jsp-config>节点的配置信息
    ...
    throws JasperException, IOException {
        ...
        JspConfig jspConfig = ctxt.getOptions().getJspConfig(); //<servlet> 节点的配置信息
        JspConfig.JspProperty jspProperty = jspConfig.findJspProperty(
                absFileName);//<jsp-config>节点的配置信息
        if (jspProperty.isXml() != null) {// 是否以XML声明的方式感知文件编码类型 ，如<?xml version="1.0" encoding="UTF-8" ?>
        ...
        if (isExternal && !isXml) {
            // JSP (standard) syntax. Use encoding specified in jsp-config  JSP的语法感知
            // if provided.
            sourceEnc = jspConfigPageEnc;//使用<jsp-config>节点的编码声明<page-encoding>
            if (sourceEnc != null) {
                return;
            }
            ...
            // XML syntax or unknown, (auto)detect encoding ... 是否以XML声明的方式感知文件编码类型 ，如<?xml version="1.0" encoding="UTF-8" ?>
        ...
        if (!isBomPresent) {
            ...
            if (sourceEnc == null) {
            	/**
            	 头部声明  <%@ page language="java" contentType="text/html; charset=UTF-8"  pageEncoding="UTF-8"%>
            	 */
                sourceEnc = getPageEncodingForJspSyntax(jspReader, startMark);//检查JSP的语法，提取页面编码
                if (sourceEnc == null) {
...
//          头部声明  <%@ page language="java" contentType="text/html; charset=UTF-8"  pageEncoding="UTF-8"%>
            ...
            if (jspReader.matches("tag ") || jspReader.matches("page")) {
                ...
                jspReader.skipSpaces();
                Attributes attrs = Parser.parseAttributes(this, jspReader);//标签的属性
    ...
    private String getPageEncodingFromDirective(Attributes attrs,
        ...
        String encoding = null;
        if (contentType != null) {
            int loc = contentType.indexOf(CHARSET);//charset=
            if (loc != -1) {
