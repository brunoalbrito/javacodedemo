...
public class Http11Processor implements ActionHook {
    ...
    public Http11Processor(int headerBufferSize, JIoEndpoint endpoint) {
        ...
        this.endpoint = endpoint;//可以取得socket
        
        request = new Request();//请求对象
        inputBuffer = new InternalInputBuffer(request, headerBufferSize);
        request.setInputBuffer(inputBuffer);

        response = new Response();//响应对象
    ...
    public void process(Socket theSocket)
    	 ...
    	 * 
            ...
            processor.endpoint = endpoint;//可以取得socket
        ...
        rp.setStage(org.apache.coyote.Constants.STAGE_PARSE);//请求信息  stage = 1
        ...
        int soTimeout = endpoint.getSoTimeout();//等于 -1

        // When using an executor, these values may return non-positive values
        int curThreads = endpoint.getCurrentThreadsBusy();//正在繁忙的线程数量
        int maxThreads = endpoint.getMaxThreads();//最大线程处理线程数量
        if (curThreads > 0 && maxThreads > 0) {
            // Only auto-disable keep-alive if the current thread usage % can be
            // calculated correctly
            if ((curThreads*100)/maxThreads > 75) {//繁忙的线程超过允许数量的 75%
                keepAliveLeft = 1;
            }
        }
        ...
        while (started && !error && keepAlive && !endpoint.isPaused()) {
            ...
            try {
                ...
                request.setStartTime(System.currentTimeMillis());//请求开始时间 
                keptAlive = true;
                if (disableUploadTimeout) {//false
                    socket.setSoTimeout(soTimeout);
                } else {
                    socket.setSoTimeout(timeout);
                }
                /**
                 * request.getMimeHeaders() === new MimeHeaders()
                 */
            ...
            // thrA.setParam(null);
            // Next request
            inputBuffer.nextRequest();//回收inputBuffer空间
            outputBuffer.nextRequest();//回收outputBuffer空间
        ...
        // Recycle
        inputBuffer.recycle();//回收inputBuffer空间
        outputBuffer.recycle();//回收outputBuffer空间
        this.socket = null;
        // Recycle ssl info
        sslSupport = null;
    }
    ...
    public void action(ActionCode actionCode, Object param) {
        ...
        } else if (actionCode == ActionCode.ACTION_START) {//开始
    ...
    protected void prepareRequest() {
        ...
        MimeHeaders headers = request.getMimeHeaders();//取得Mime类型信息

        // Check connection header
        MessageBytes connectionValueMB = headers.getValue("connection");//连接头
        ...
        if (transferEncodingValueMB != null) {
            ...
            encodingName = transferEncodingValue.substring(startPos)
                .toLowerCase().trim();
            if (!addInputFilter(inputFilters, encodingName)) {//添加输入过滤器 
                // Unsupported transfer encoding
                error = true;
                // 501 - Unimplemented
                if (log.isDebugEnabled()) {
        ...
        long contentLength = request.getContentLengthLong();//实体内容长度  content-length
        if (contentLength >= 0) {
