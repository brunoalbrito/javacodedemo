...
public class Request
     ...
     * Coyote request.
     */
    protected org.apache.coyote.Request coyoteRequest;//原生对象
    ...
    public Context getContext() {
    	//org.apache.catalina.connector.CoyoteAdapter.service(...)设置
        return (this.context);
    }
    ...
    public String getParameter(String name) {
        ...
        if (!parametersParsed)
            parseParameters();//!!!!!!解析url中的参数，和表单中的参数
    ...
    public RequestDispatcher getRequestDispatcher(String path) {
        ...
        if (path == null)
        	...
        	//org.apache.catalina.core.StandardContext.getServletContext().getRequestDispatcher(path)
        	//org.apache.catalina.core.ApplicationContext.getRequestDispatcher(path)
            return (context.getServletContext().getRequestDispatcher(path));//!!!

        // Convert a request-relative path to a context-relative one
        String servletPath = (String) getAttribute(Globals.INCLUDE_SERVLET_PATH_ATTR);// javax.servlet.include.servlet_path
        if (servletPath == null)
    ...
    public void removeAttribute(String name) {
        ...
        for (int i = 0; i < listeners.length; i++) {
            ...
            try {
                listener.attributeRemoved(event);//触发“请求级别的监听器”中的属性删除事件
            } catch (Throwable t) {
    ...
    public void setAttribute(String name, Object value) {
        ...
        // Notify interested application event listeners
        Object listeners[] = context.getApplicationEventListeners();//“请求级别的监听器”
        if ((listeners == null) || (listeners.length == 0))
            return;
        ...
        for (int i = 0; i < listeners.length; i++) {
            ...
            try {
                if (replaced) {
                    listener.attributeReplaced(event);//触发“请求级别的监听器”中的属性替换事件
                } else {
                    listener.attributeAdded(event);//触发“请求级别的监听器”中的属性添加事件
                }
            } catch (Throwable t) {
                context.getLogger().error(sm.getString("coyoteRequest.attributeEvent"), t);
                // Error valve will pick this execption up and display it to user
    ...
    public HttpSession getSession() {
        Session session = doGetSession(true);//!!!!!!!!! 取得Session对象
        if (session != null) {
    ...
    protected Session doGetSession(boolean create) {
        ...
        if (context != null)
            manager = context.getManager();//!!!!!!!!!!!取得管理器
        if (manager == null)
            return (null);      // Sessions are not supported
        if (requestedSessionId != null) {
            try {
                session = manager.findSession(requestedSessionId);//取得Session
            } catch (IOException e) {
                session = null;
            }
            if ((session != null) && !session.isValid())
        ...
        if (connector.getEmptySessionPath() 
            ...
            session = manager.createSession(null);//!!!!! 取得Session对象
        }

        // Creating a new session cookie based on that session
        if ((session != null) && (getContext() != null)
               && getContext().getCookies()) {
            String scName = context.getSessionCookieName();//org.apache.catalina.core.StandardContext.getSessionCookieName()
            if (scName == null) {
                scName = Globals.SESSION_COOKIE_NAME;
            }
    ...
    protected void configureSessionCookie(Cookie cookie) {
        ...
        Context ctxt = getContext();// org.apache.catalina.core.StandardContext
    ...
    protected void parseCookies() {
        ...
        Cookies serverCookies = coyoteRequest.getCookies();// 从原生对象coyoteRequest中读取Cookie
        int count = serverCookies.getCookieCount();//Cookie数量
        if (count <= 0)
            return;
        ...
        for (int i = 0; i < count; i++) {
            ...
            try {
                ...
                */
                Cookie cookie = new Cookie(scookie.getName().toString(),null);//!!!!!!!!!创建cookie
    ...
    protected void parseParameters() {
        ...
        Parameters parameters = coyoteRequest.getParameters();//原生对象coyoteRequest  （org.apache.catalina.connector.Request）
        // Set this every time in case limit has been changed via JMX
        parameters.setLimit(getConnector().getMaxParameterCount());

        // getCharacterEncoding() may have been overridden to search for
        // hidden form field containing request encoding
        String enc = getCharacterEncoding();// ... coyoteRequest.getCharacterEncoding()
        ...
        parameters.handleQueryParameters();//!!!!!!!处理URL的参数
        ...
        try {
            if (len > 0) {
                int maxPostSize = connector.getMaxPostSize();//!!!!!!!!最大POST大小
                ...
                parameters.processParameters(formData, 0, len);//处理表单中的字段的数据
