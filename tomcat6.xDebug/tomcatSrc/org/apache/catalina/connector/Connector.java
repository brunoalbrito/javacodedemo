    ...
    implements Lifecycle, MBeanRegistration
{
    ...
    public Connector(String protocol)
        ...
        try {
            ...
            /**
            	this.protocolHandler =  new org.apache.coyote.http11.Http11Protocol()
        		this.protocolHandler =  new org.apache.jk.server.JkCoyoteHandler()
             */
     ...
     * 在构造函数中进行实例化
     * 根据Connector标签中的protocol值，实例化相应的对象
            org.apache.coyote.http11.Http11Protocol
    ...
    protected ObjectName createObjectName(String domain, String type)
        ...
        StringBuilder sb = new StringBuilder(domain);//<Engine name="Catalina"   ---domain==Catalina
        sb.append(":type=");
        sb.append(type);
        sb.append(",port=");
        sb.append(getPort());//Catalina:type=Connector,port=8080
        if (addressObj != null) {
            String address = addressObj.toString();
            if (address.length() > 0) {
                sb.append(",address=");
                sb.append(ObjectName.quote(address));//Catalina:type=Connector,port=8080,address=127.0.0.1
            }
        }
        ...
        throws LifecycleException
    {
        ...
        if( oname == null && (container instanceof StandardEngine)) {//执行进来
            try {
                // we are loaded directly, via API - and no name was given to us
            	/**
            	 * 什么时候设置container的？
            	 * //把Container设置进去 Connector
            	 * //把Service设置进去Connector
            	 * org.apache.catalina.core.StandardService.addConnector(Connector connector)
            	 * org.apache.catalina.core.StandardService.setContainer(Container container)，保证后面添加的Container也能添加到Connector
            	 * 以上两个都有执行设置，做保证作用
            	 * 
            	 */
                StandardEngine cb=(StandardEngine)container;//org.apache.catalina.core.StandardEngine
                System.out.println("org.apache.catalina.connector.Connector.initialize###StandardEngine.getName------>"+cb.getName());//cb.getName()===引擎名称===Catalina
                //oname = Catalina:type=Connector,port=xxxx,address=xxxxx
                oname = createObjectName(cb.getName(), "Connector");//createObjectName("Catalina", "Connector"); --->new ObjectName("Catalina:type=Connector,port=8080");
                /**
                 * cb.getName()等于 Catalina
                 * StandardEngine.getName(),为什么是叫Catalina？
                 * 		原因是：在标签中定义的。<Engine name="Catalina" defaultHost="localhost">
                 * 
                 * oname是什么？
                 * 		是KEY吗？
                 * 
                 * 会调用org.apache.catalina.connector.Connector.preRegister(....) 这个是JMX的回调
                 * 会设置this.domain = "Catalina"
                 */
                Registry.getRegistry(null, null)
                    .registerComponent(this, oname, null);//往对象管理中注册对象，并调用本类内部的preRegister、postRegister方法
                controller=oname;
            } catch (Exception e) {
        ...
        /**
         * protocolHandler = new org.apache.coyote.http11.Http11Protocol(),通过反射映射标签中的属性，动态创建对象protocolHandler
         * protocolHandler = new org.apache.jk.server.JkCoyoteHandler()，通过反射映射标签中的属性，动态创建对象protocolHandler
         */
        protocolHandler.setAdapter(adapter);//setAdapter(adapter)
...
//        	org.apache.jk.server.JkCoyoteHandler.init();
    ...
    public void start() throws LifecycleException {
        ...
        try {
            ...
            org.apache.jk.server.JkCoyoteHandler().start
        	 */
         ...
         *  注册了包装器Wrapper
         */
        System.out.println("######org.apache.catalina.connector.Connector.start()######"+ this.domain );
        if( this.domain != null ) {//Catalina
        	
        	/**
        	 	domain什么时候设置的？
        	 		在org.apache.catalina.connector.Connector.preRegister(MBeanServer server, ObjectName name)设置的
        	 	什么时候调用的preRegister？
        	 		在org.apache.catalina.connector.Connector.initialize()调用的
        	 	...
        	 	很重要
        	 		...
        	 		里面注册了：registerEngine
        	 		里面注册了：registerHost
        	 		里面注册了：registerWrapper
        	 */
        	/**
             ...
             * 		mBeanServer.addNotificationListener(objectName, mapperListener, null, null);
             ...
             mapper.addWrapper(hostName, contextName, mappings[i], wrapper,
                              jspWildCard);
             */
            mapperListener.init();//读取JMX中注册的信息":type=Engine"/":type=Host"/"j2eeType=WebModule"/"j2eeType=Servlet"，构造路由树，创建路由Mapper!!!!!!!!!!!!!!!!!!!
            try {
                ObjectName mapperOname = createObjectName(this.domain,"Mapper");//执行进来
                if (log.isDebugEnabled())
    ...
    public ObjectName preRegister(MBeanServer server,
        ...
        oname=name;
        mserver=server;
        domain=name.getDomain();//这边设置domain
        return name;
    }
    ...
    protected void findContainer() {
        try {
            ...
            if( mserver.isRegistered(parentName )) {//把connector 对象节点，添加到 Service节点下
            ...
            ObjectName engName=new ObjectName( domain + ":" + "type=Engine");// 取得 Engine 对象节点，并返回
