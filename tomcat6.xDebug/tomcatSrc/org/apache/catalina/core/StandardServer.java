...
public final class StandardServer
    implements Lifecycle, Server, MBeanRegistration 
 {
    ...
    public void await() {
        ...
        try {
            awaitSocket =
                new ServerSocket(port, 1,
                                 InetAddress.getByName("localhost"));//创建Socket 端口是：8005
        } catch (IOException e) {
        ...
        try {
            ...
            while (!stopAwait) {
                ...
                Socket socket = null;
                StringBuilder command = new StringBuilder();//命令字符串
                try {
                    InputStream stream = null;
                    try {
                        socket = serverSocket.accept();//监听socket
                        socket.setSoTimeout(10 * 1000);  // Ten seconds  十秒
                        stream = socket.getInputStream();//读取输入流
                ...
                boolean match = command.toString().equals(shutdown);//SHUTDOWN
                if (match) {
                    break;
                } else
    ...
    public void start() throws LifecycleException {
        ...
        lifecycle.fireLifecycleEvent(BEFORE_START_EVENT, null);//迭代事件处理器列表，触发 BEFORE_START_EVENT 事件
        //触发本标签的事件监听器
        lifecycle.fireLifecycleEvent(START_EVENT, null);//迭代事件处理器列表，触发 START_EVENT 事件
        started = true;
        ...
        synchronized (services) {
            for (int i = 0; i < services.length; i++) {
                if (services[i] instanceof Lifecycle)
                	//!!!!!!!!!!!!!(new org.apache.catalina.core.StandardService()).start()
                    ((Lifecycle) services[i]).start();
            }
        }

        // Notify our interested LifecycleListeners
        lifecycle.fireLifecycleEvent(AFTER_START_EVENT, null);//迭代事件处理器列表，触发 AFTER_START_EVENT 事件
        ...
        throws LifecycleException 
    {
        ...
        /**
         * (new LifecycleSupport(this)).fireLifecycleEvent(INIT_EVENT, null);
         * 触发本标签的事件监听器
         * 
         * 加载类字节码到内存中：
         * 	org.apache.jasper.compiler.JspRuntimeContext JSP运行时上下文
         */
        lifecycle.fireLifecycleEvent(INIT_EVENT, null);//初始化事件
        initialized = true;
        ...
        for (int i = 0; i < services.length; i++) {
        	/**
        	 * 只有一个
        	 * 迭代调用StandardService初始化
        	 * org.apache.catalina.core.StandardService.initialize();
        	 * 
        	 */
            services[i].initialize();//循环调用 initialize
        }
    }
