...
public abstract class ContainerBase
    ...
    public void addChild(Container child) {
        if (Globals.IS_SECURITY_ENABLED) {
            ...
            addChildInternal(child);//这里面会触发 ((Lifecycle) child).start();方法!!!!!!!!
        }
    }
        ...
        synchronized(children) {
            ...
            if (started && startChildren && (child instanceof Lifecycle)) {
                ...
                try {
                	 ...
                	 * !!!!!!!!!!!!!!
                	 */
                    ((Lifecycle) child).start();//触发方法!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  org.apache.catalina.core.StandardContext
                    success = true;
                } catch (LifecycleException e) {
    ...
    public synchronized void start() throws LifecycleException {
        ...
        // Notify our interested LifecycleListeners 触发标签的指定事件的监听器
        lifecycle.fireLifecycleEvent(BEFORE_START_EVENT, null);//执行进来，触发事件处理器
         ...
         * 
         * 子类一：
         * 		org.apache.catalina.core.StandardEngine-->触发StandardHost事件监听器
         * 子类二：这二是没有执行的
         * 		org.apache.catalina.core.StandardHost-->触发StandardContext事件监听器
         */
        Container children[] = findChildren();//取得org.apache.catalina.core.StandardHost列表 org.apache.catalina.core.StandardContext
        for (int i = 0; i < children.length; i++) {//StandardHost没有子标签，所以没有执行
            if (children[i] instanceof Lifecycle)
            	//只有一个走：class org.apache.catalina.core.StandardEngine-----有进来-------class org.apache.catalina.core.StandardHost
            	System.out.println(this.getClass()+"-----有进来-------"+children[i].getClass());
            	/**
            	 * 		StandardEngine ---> Host标签的启动 org.apache.catalina.core.StandardHost.start();
            	 * 
            	 */
        ...
        /**
         	 当org.apache.catalina.core.StandardEngine.start() 触发时：
         	         在Digester代码内部有添加lifecycle(org.apache.catalina.startup.EngineConfig)，虽然在server.xml文件中看不到
         	 	触发Engine标签事件监听器org.apache.catalina.startup.EngineConfig  ，记录启动日志
         	   
         	   
		         当org.apache.catalina.core.StandardHost.start() 触发时：
		         	在Digester代码内部有添加lifecycle(org.apache.catalina.startup.HostConfig)，虽然在server.xml文件中看不到
		          
		         很重要：!!!
			          当触发者是： org.apache.catalina.core.StandardHost.start();
			          会执行org.apache.catalina.startup.HostConfig.start();，这里面会有三种方式检查到达发布了多少应用，并添加到host节点上。这就是动态添加虚拟目录
			          	迭代1、应用的配置文件或者    2、应用目录或者   3、war应用发布应用,
         */
...
//    	fireContainerEvent("addApplicationListener", "cn.java.listener.MyListener");
        if (listeners.size() < 1)
            return;
    ...
    protected class ContainerBackgroundProcessor implements Runnable {
        ...
        protected void processChildren(Container container, ClassLoader cl) {
            try {
                ...
                container.backgroundProcess();//执行
            } catch (Throwable t) {
                log.error("Exception invoking periodic operation: ", t);
            } finally {
                Thread.currentThread().setContextClassLoader(cl);
            }
            Container[] children = container.findChildren();//迭代Engine标签的子标签Host
            for (int i = 0; i < children.length; i++) {
                if (children[i].getBackgroundProcessorDelay() <= 0) {
                    processChildren(children[i], cl);//递归执行子标签的背景处理线程
                }
            }
        }
