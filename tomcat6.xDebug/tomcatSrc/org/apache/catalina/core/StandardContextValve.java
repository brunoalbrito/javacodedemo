...
final class StandardContextValve
    ...
    public final void invoke(Request request, Response response)
        ...
        Wrapper wrapper = request.getWrapper();// 这个就是 <servlet>标签解析处理的对象，就是Servlet对象
        if (wrapper == null) {
            notFound(response);
            return;
        ...
        Object instances[] = context.getApplicationEventListeners();//取得“请求级别的监听器”列表
        ...
        if ((instances != null) 
                && (instances.length > 0)) {
        	//取得监听器列表
        	//org.apache.catalina.core.ApplicationContext === (StandardContext) container).getServletContext()
         ...
         * getPipeline() === new StandardPipeline(StandardWrapper);//类的聚合变量
         * getFirst()===new StandardWrapperValve()//在StandardWrapper的构造函数赋值
         * getFirst().invoke(request, response); //===StandardWrapperValve.invoke(request, response)
         */
        // Ask 
