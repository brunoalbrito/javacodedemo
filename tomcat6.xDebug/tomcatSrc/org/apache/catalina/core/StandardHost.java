    ...
    implements Host  
 {
    ...
    public synchronized void start() throws LifecycleException {
        ...
        if( realm == null ) {
            ...
            try {
                realmName=new ObjectName( domain + ":type=Realm,host=" + getName());//localhost,www.aaa.com
        ...
        /**
             	触发事件：org.apache.catalina.startup.HostConfig.start();
             		这里面会有三种方式检查到达发布了多少应用，
	             		创建对象：  创建org.apache.catalina.core.StandardContext对象，并添加到host节点上，
	             		调用对象方法： 并触发org.apache.catalina.core.StandardContext的start();方法
	                 	org.apache.catalina.core.StandardContext.start();
	                 	{
	                 	    ...
	                 	    lifecycle.fireLifecycleEvent(START_EVENT, null);//触发事件处理器 org.apache.catalina.startup.ContextConfig.lifecycleEvent(LifecycleEvent event) 触发解析/WEB-INF/web.xml和conf/web.xml
	
	                 	}
		 */
        System.out.println(this.getClass()+":start(),这里会处理加载配置文件...");
        super.start();//执行进来,里面处理上下文件的配置
    ...
    public void init() {
        ...
        if( oname==null ) {
            ...
            try {
                 ...
                 * Catalina:type=Host,host=www.aaa.com
                 */
