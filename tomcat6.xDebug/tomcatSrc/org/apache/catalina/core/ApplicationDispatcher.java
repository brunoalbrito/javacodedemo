...
final class ApplicationDispatcher
    ...
    public ApplicationDispatcher
        ...
        this.queryString = queryString;// key1=value1&key2=value2
        this.name = name; //null
        ...
        throws ServletException, IOException
    {
        if (Globals.IS_SECURITY_ENABLED) {
            ...
            doForward(request,response);//内部转发
        }
    }
...
//        	<jsp:include page="/WEB-INF/servletView/admin/Common-footer.jsp" flush="true"/>
        ...
        throws ServletException, IOException
    {
        ...
        /**
         	state.outerResponse === ServletResponseWrapperInclude
         	wrapper = new ApplicationHttpResponse(ResponseFacade,true);
	        state.outerResponse.setResponse(wrapper); 给  ResponseFacade 包装一层
	        state.wrapResponse = wrapper; //修改成新的
         */
...
//            <jsp:include page="/WEB-INF/servletView/admin/Common-footer.jsp" flush="true"/>
            /**
             	!!! state.outerResponse === ServletResponseWrapperInclude
             	state.outerResponse.setResponse(new ApplicationHttpResponse(ResponseFacade,true)); // 给  ResponseFacade 包装一层
             */
            invoke(state.outerRequest, state.outerResponse, state); // !!!!
        }
...
//    	<jsp:include page="/WEB-INF/servletView/admin/Common-footer.jsp" flush="true"/>
        ...
        try {
            ...
            if ((servlet != null) && (filterChain != null)) {
               //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
               //!!!!!!!执行过滤器，Servlet在过滤器链条的末端!!!!!!!!!
               //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
               filterChain.doFilter(request, response);// 执行过滤器，Servlet在过滤器链条的末端
             }
    ...
    private ServletResponse wrapResponse(State state) {
        ...
        ServletResponse previous = null;
        // 如果是 <jsp:include 标签 ，那么   current == state.outerResponse === ServletResponseWrapperInclude
        ServletResponse current = state.outerResponse;
        while (current != null) {
            ...
            if (current instanceof Response)
                break;
            previous = current; // 如果是 <jsp:include 标签   previous ==  ServletResponseWrapperInclude
            current = ((ServletResponseWrapper) current).getResponse(); // 如果是 <jsp:include 标签  current == org.apache.catalina.connector.ResponseFacade
        }
...
//         如果是 <jsp:include 标签,   wrapper = new ApplicationHttpResponse(ResponseFacade,true);
        	wrapper =
        ...
        if (previous == null)
            ...
            ((ServletResponseWrapper) previous).setResponse(wrapper);// 如果是 <jsp:include 标签 ,
