...
public class StandardService
        implements Lifecycle, Service, MBeanRegistration 
 {
    ...
    public void setContainer(Container container) {
        ...
        synchronized (connectors) {
            for (int i = 0; i < connectors.length; i++)
                connectors[i].setContainer(this.container);//把container设置进connectors
        }
    ...
    public void addConnector(Connector connector) {
        ...
        synchronized (connectors) {
            connector.setContainer(this.container);//把Container设置进去
            connector.setService(this);//把Service设置进去
    ...
    public void start() throws LifecycleException {
        ...
        /**
         * 触发Service标签的事件监听器,BEFORE_START_EVENT事件
         */
        ...
        /**
         * 触发Service标签的事件监听器,START_EVENT事件
         */
         ...
         * 默认文件中只有一个Engine标签
         */
        if (container != null) {//迭代启动container*********org.apache.catalina.core.StandardEngine.start();
        ...
        /**
         * 默认配置文件中没有executors标签
         */
        synchronized (executors) {//迭代启动executors
            for ( int i=0; i<executors.size(); i++ ) {
                //(new org.apache.catalina.core.StandardThreadExecutor()).start();
        ...
        /**
         * 默认配置文件中，有两个Connector标签
        
         tomcat监听的端口，http协议
         <Connector port="8080" protocol="HTTP/1.1"  connectionTimeout="20000"  redirectPort="8443" />
       
         apache和tomcat的通信端口，AJP协议
         <Connector port="8009" protocol="AJP/1.3" redirectPort="8443" />
         
 			相依的类：Connector con = new org.apache.catalina.connector.Connector(attributes.getValue("protocol"));
                            相依的类：Connector con = new org.apache.catalina.connector.Connector("HTTP/1.1");
        	相依的类：Connector con = new org.apache.catalina.connector.Connector("AJP/1.3");
         */
        synchronized (connectors) {
            for (int i = 0; i < connectors.length; i++) {
                try {
                	//(new org.apache.catalina.connector.Connector()).start()
            ...
            throws LifecycleException
    {
        ...
        if( oname==null ) {
            try {
            	...
            	/**
            	 * 标签：Server/Service/Engine
            	 * 相应的类：org.apache.catalina.core.StandardEngine
            	 */
            	// 这个等价： engine = org.apache.catalina.core.StandardEngine = this.getContainer();
                ...
                /**
                 * 注册到Bean管理容器中
                 * (new Registry()).registerComponent(this, oname, null);
                 */
                ...
                /**
                 * 
                 * 标签：Server/Service/Executor
                 * 相应的类：org.apache.catalina.core.StandardThreadExecutor
                 * 
                 */
                Executor[] executors = findExecutors();//往对象管理器中注册executors
                for (int i = 0; i < executors.length; i++) {
                    ObjectName executorObjectName = 
                        new ObjectName(domain + ":type=Executor,name=" + executors[i].getName());
                    Registry.getRegistry(null, null)
        ...
        synchronized (connectors) {//迭代Connectors列表，进行初始化
            for (int i = 0; i < connectors.length; i++) {
                try {
                    connectors[i].initialize();//初始化连接 org.apache.catalina.connector.Connector.initialize();
                } catch (Exception e) {
