...
public final class ApplicationFilterFactory {
    ...
    public ApplicationFilterChain createFilterChain
        ...
        if (request instanceof Request) {
            Request req = (Request) request;//转义
            comet = req.isComet();
            if (Globals.IS_SECURITY_ENABLED) {//安全模式
                ...
                filterChain = (ApplicationFilterChain) req.getFilterChain();//过滤器链条
                if (filterChain == null) {
                    filterChain = new ApplicationFilterChain();//创建过滤器链条!!!!!!!!!!!!!!!!!!
                    req.setFilterChain(filterChain);
                }
            }
        } else {
        ...
        filterChain.setServlet(servlet);//Servlet对象设置进入过滤器链条!!!!!!!!!!!
        ...
        FilterMap filterMaps[] = context.findFilterMaps();//过滤器映射列表!!!!!!!!!!!!!!
        ...
        for (int i = 0; i < filterMaps.length; i++) {
            if (!matchDispatcher(filterMaps[i] ,dispatcher)) {//不符合过滤器规则
                continue;
            }
            if (!matchFiltersURL(filterMaps[i], requestPath))//不符合过滤器规则
                continue;
            ApplicationFilterConfig filterConfig = (ApplicationFilterConfig)
                context.findFilterConfig(filterMaps[i].getFilterName());//某个过滤器的配置
            ...
            if (comet) {
                ...
                filterChain.addFilter(filterConfig);//某个过滤器的配置信息
            }
        }
        ...
        for (int i = 0; i < filterMaps.length; i++) {
            ...
            if (comet) {
                ...
                filterChain.addFilter(filterConfig);//符合条件的过滤器配置添加进去!!!!!!!!!!!!!
            }
        }
