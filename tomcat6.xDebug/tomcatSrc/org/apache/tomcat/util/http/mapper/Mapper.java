...
public final class Mapper {
    ...
    public void setDefaultHostName(String defaultHostName) {
        this.defaultHostName = defaultHostName;//localhost
    }
    ...
    public synchronized void addHost(String name, String[] aliases,
        ...
        ContextList contextList = new ContextList();//上下文列表
        newHost.name = name;
        ...
        for (int i = 0; i < aliases.length; i++) {//添加别名列表
    ...
    public void addContext
        ...
        int pos = find(hosts, hostName);//比对虚拟主机
        if( pos <0 ) {//虚拟主机记录项不存在,就添加进去
        ...
        if (host.name.equals(hostName)) {
            int slashCount = slashCount(path);//目录的深度，层数
            synchronized (host) {
  ...
  //localhost,/,<servlet-mapping>的配置，org.apache.jasper.servlet.JspServlet，false
    ...
    protected void addWrapper(Context context, String path, Object wrapper,
        ...
        synchronized (context) {
            ...
            newWrapper.object = wrapper;//org.apache.jasper.servlet.JspServlet和org.apache.catalina.servlets.DefaultServlet
            newWrapper.jspWildCard = jspWildCard;//true false
            if (path.endsWith("/*")) {
                // Wildcard wrapper
     ...
     *
     * @param host Virtual host name  虚拟主机名
     * @param uri URI	URL的信息
    ...
    public void map(MessageBytes host, MessageBytes uri,
        ...
        if (host.isNull()) {
            host.getCharChunk().append(defaultHostName);//使用默认虚拟主机
        }
        host.toChars();
        uri.toChars();//   /docs
        /**
    ...
    private final void internalMap(CharChunk host, CharChunk uri,
        ...
        /**
         * 虚拟主机列表hosts
         * 是在org.apache.catalina.connector.Connector.start()中的mapperListener.init()反向注入的
         * 
         */
        // Virtual host mapping
        //没有虚拟主机头
        if (mappingData.host == null) {
            Host[] hosts = this.hosts;//虚拟主机列表Host[0]
            int pos = findIgnoreCase(hosts, host);//查找是否有配置访问的虚拟主机
            if ((pos != -1) && (host.equalsIgnoreCase(hosts[pos].name))) {//匹配到虚拟主机头
                mappingData.host = hosts[pos].object;//填写虚拟主机头
                contexts = hosts[pos].contextList.contexts;//虚拟主机的上下文列表
                nesting = hosts[pos].contextList.nesting;
            } else {
                if (defaultHostName == null) {
                    return;
                }
                pos = find(hosts, defaultHostName);//defaultHostName===localhost
                if ((pos != -1) && (defaultHostName.equals(hosts[pos].name))) {
                    mappingData.host = hosts[pos].object;//使用默认的虚拟主机头
        ...
        // Context mapping
        /**
         * 根据URI查找是否有符合的context
         * 如在host标签下有设置context，那么就进行匹配。
         */
        if (mappingData.context == null) {
        	System.out.println("##########mappingData.contex###########");
            int pos = find(contexts, uri);//如    /dirname1/
            if (pos == -1) {
                return;
            }
            ...
            if (context != null) {
                mappingData.context = context.object;//把上下文对象设置进去 。 如:    /dirname1/
        ...
        if ((context != null) && (mappingData.wrapper == null)) {//
            ...
            System.out.println("############org.apache.tomcat.util.http.mapper.Mapper.internalMap############");
        }
    ...
    private final void internalMapWrapper(Context context, CharChunk path,
        ...
        int length = context.name.length();//如 ：/example1/
        ...
        path.setOffset(servletPath);//设置Servlet路径

        // Rule 1 -- Exact Match	扩展匹配？？？
        /**
         * org.apache.catalina.core.StandardContext.exactWrappers  这是对象列表
         */
        Wrapper[] exactWrappers = context.exactWrappers;//context.exactWrappers什么时候设置的呀？？
        ...
        // Rule 3 -- Extension Match	扩展名配置!!!!!!!!
        Wrapper[] extensionWrappers = context.extensionWrappers;//
        if (mappingData.wrapper == null && !checkJspWelcomeFiles) {
            internalMapExtensionWrapper(extensionWrappers, path, mappingData);
        }

        // Rule 4 -- Welcome resources processing for servlets  使用欢迎的文件来匹配
        if (mappingData.wrapper == null) {
            boolean checkWelcomeFiles = checkJspWelcomeFiles;
            if (!checkWelcomeFiles) {//没有欢迎文件
                char[] buf = path.getBuffer();
                checkWelcomeFiles = (buf[pathEnd - 1] == '/');
            }
            if (checkWelcomeFiles) {//有欢迎文件
        ...
        // Rule 7 -- Default servlet     没有匹配到，并且没有欢迎页面。那么，就使用默认的Servlet
        if (mappingData.wrapper == null && !checkJspWelcomeFiles) {
            if (context.defaultWrapper != null) {
                mappingData.wrapper = context.defaultWrapper.object;//上下文默认的Wrapper
                mappingData.requestPath.setChars
    ...
    private static final boolean insertMap
        ...
        if ((pos != -1) && (newElement.name.equals(oldMap[pos].name))) {//有存在虚拟主机记录
            return false;
        }
