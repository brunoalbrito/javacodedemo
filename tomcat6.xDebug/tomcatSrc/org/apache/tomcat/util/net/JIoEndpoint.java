    ...
    protected class Worker implements Runnable {
        ...
        public void run() {
            ...
            while (running) {//false
                 ...
                 * 
                 * handler.process(socket)创建Http11Processor对象
                 * 
                 */
...
//                org.apache.coyote.http11.Http11Protocol.Http11ConnectionHandler.process(socket)
                ...
                // Finish up this request
                socket = null;
                recycleWorkerThread(this);//回收工作进程
    ...
    public void init()
        ...
        if (serverSocket == null) {
            try {
                if (address == null) {
                	//org.apache.tomcat.util.net.DefaultServerSocketFactory.createSocket(port, backlog);
                    serverSocket = serverSocketFactory.createSocket(port, backlog);//new ServerSocket (port, backlog);
//                    serverSocket = new ServerSocket (port, backlog);
    ...
    public void start()
        ...
        if (!running) {
             ...
             */
            if (executor == null) {
                workers = new WorkerStack(maxThreads);//初始化工作栈，栈元素可以有200
            }
             ...
             * Acceptor是内部类
             * 
             * acceptorThreadCount = 1;//在init()中置为1
             */
    ...
    protected Worker createWorkerThread() {
        ...
        synchronized (workers) {//锁定工作线程栈
            if (workers.size() > 0) {
                curThreadsBusy++;
                return workers.pop();//弹出一个工作线程
            }
    ...
    protected Worker getWorkerThread() {
        ...
        synchronized (workers) {
            ...
            while ((workerThread = createWorkerThread()) == null) {// 没有取得工作线程的话
                try {
                    workers.wait();//工作栈进入等待，需要等待其他线程唤醒
                } catch (InterruptedException e) {
                    // Ignore
                }
            }
    ...
    protected boolean processSocket(Socket socket) {
        try {
            if (executor == null) {//执行这边
            	 ...
            	 * (new Worker()).start().wait().assign(socket);
            	 * 唤醒调用处理
            	 * 
            	 */
