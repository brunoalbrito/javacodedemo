   ...
   /**
    ...
    public XMLReader getXMLReader() throws SAXException {
        if (reader == null){
        	 ...
        	 */
            reader = getParser().getXMLReader();//XMLReader
        }

        /**
         * 把Handler设置成自己，so important
         */
    ...
    public void endElement(String namespaceURI, String localName,
        ...
        List rules = (List) matches.pop();//把推入堆栈的规则执行对象列表弹出
    ...
    public void startElement(String namespaceURI, String localName,
        ...
        bodyTexts.push(bodyText);	//	推入堆栈bodyTexts中
        if (debug) {
        ...
        if (match.length() > 0) {//如果是二级的标签
            sb.append('/');
        }
        sb.append(name); // /Server 标签
        match = sb.toString();// Server 标签
        if (debug) {
        ...
        /**
         * RulesBase.match(namespaceURI, match);
         * List rulesList = RulesBase.lookup(namespaceURI, match);
         * List list = (List) RulesBase.cache.get(match);//....Hashmap.get(match)
         * 取得指定key的对象列表
         */
        List rules = getRules().match(namespaceURI, match);
        matches.push(rules);//把对象列表推入堆栈  matches  ArrayStack
        if ((rules != null) && (rules.size() > 0)) {
            for (int i = 0; i < rules.size(); i++) {
                try {
                    Rule rule = (Rule) rules.get(i);
                    ...
                    /**
                     * 迭代调用指定key中的对象列表中的对象方法begin
                     * 如：ObjectCreateRule.begin()
                     * 			创建对象，并推入堆栈
                     * 	  SetPropertiesRule.begin()
                     * 			把匹配到的开始标签中的属性映射到创建的对象中
                     * 	  SetNextRule.begin()
                     * 			没有执行方法
                     */
    ...
    public Object parse(InputSource input) throws IOException, SAXException {
         ...
         * 等于执行如下
         * 
         * 把XMLReader的Handler设置成this，即本身作为XML解析的回调。so important
         * 	故：在 startDocument()，创建日志对象
         * 		在startElement,触发通过addObjectCreate和addSetProperties绑定的指定标签的行为。（创建对象和把取得DOM的属性反射注入到对象的属性中）
         * 		在endElement中，触发通过addSetNext绑定的指定标签的行为。（把堆栈顶的元素，反射注入到第二级的堆栈的元素中）
         * ===(getParser().getXMLReader()).parse(input);
         * ===(getFactory().newSAXParser().getXMLReader()).parse(input);
         * ===(SAXParserFactory.newInstance().newSAXParser().getXMLReader()).parse(input);
    ...
    public Object parse(InputStream input) throws IOException, SAXException {
        ...
        configure();//配置日志对象
        InputSource is = new InputSource(input);//org.xml.sax.InputSource() == org.xml.sax.byteStream = input
     ...
     * addRule("Server/Service/Connector", new ConnectorCreateRule())
     */
    public void addRule(String pattern, Rule rule) {

        rule.setDigester(this);
        /*
         * RulesBase，保存着HashMap，key是标签匹配的表达式，值是数组列表，一个标签匹配的表达式，可以有多个处理方法。
         * RulesBase，保存着ArrayList，保存的是所有的处理方法，没有区分标签匹配的表达式
         	new RulesBase().add(pattern, rule);
	        List list = (List) cache.get(pattern);
	        if (list == null) {
	            list = new ArrayList();
	            cache.put(pattern, list);//HashMap.put(pattern, list);
	        }
	        list.add(rule);
          
           rules.add(rule);  //ArrayList.add(rule);
         */
     ...
     * @see ObjectCreateRule
     * 
     * 这个方法就是定义了一个创建对象的规则：匹配的规则（遇到server.xml里什么样的表达式），
     * 默认要实例化的类名，默认的一个属性名（xml配置属性）。就是说如果server.xml里遇到跟规
     * 则相匹配的项，如果没有设定指定attributeName属性的值，则使用这里设置的默认的类。 
     */
    public void addObjectCreate(String pattern, String className,
                                String attributeName) {

    	/**
    	 * addRule("Server",new ObjectCreateRule("org.apache.catalina.core.StandardServer", "className"));
    	 */
        addRule(pattern,
     ...
     * @see SetNextRule
     * 
     * 第一个参数依旧是匹配的规则，第二个参数是要调用父元素的方法名，第三个参数是方法需要的参数类型。
     * 
     */
    public void addSetNext(String pattern, String methodName,
                           String paramType) {
    	/**
    	 * addRule("Server",new SetNextRule("setServer", "org.apache.catalina.Server"));
    	 */
        addRule(pattern,
    ...
    public void addSetProperties(String pattern) {
    	...
    	/**
    	 * addRule("Server",new SetPropertiesRule())
    	 */
        addRule(pattern,
    ...
    public void push(Object object) {
        ...
        stack.push(object);//ArrayStack.push(),ArrayList.add()
