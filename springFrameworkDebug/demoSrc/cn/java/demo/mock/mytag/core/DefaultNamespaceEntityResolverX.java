...
public class DefaultNamespaceEntityResolverX implements EntityResolver {
	...
	public InputSource resolveEntity(String publicId, String systemId) throws SAXException, IOException {
		...
		String resourceName = "cn/java/demo/mock/mytag/schemas/META-INF/mytag.schemas";
		...
		if (systemId != null) {
			if (systemId.endsWith(DTD_SUFFIX)) {
				...
				// 读取属性文件
				Enumeration<URL> urls = DefaultNamespaceEntityResolverX.class.getClassLoader().getResources(resourceName); // 
				...
				// 获取指定key的值
				// systemId == "http://www.janchou.org/schema/mock/mytag/spring-mytag-3.0.xsd"
				// resourceLocation = "cn/java/demo/mock/mytag/schemas/spring-mytag-3.0.xsd"
				String resourceLocation = schemaMappings.get(systemId); 
				// 读取xsd文件
				InputStream inputStream = DefaultNamespaceEntityResolverX.class.getClassLoader().getResourceAsStream(resourceLocation);
		...
		if (source == null && systemId != null) { // 没有识别到
			...
			if (resourcePath != null) {
				source = new InputSource(DefaultNamespaceEntityResolverX.class.getResourceAsStream(resourcePath));
				source.setPublicId(publicId);
				source.setSystemId(systemId);
			}
		}
