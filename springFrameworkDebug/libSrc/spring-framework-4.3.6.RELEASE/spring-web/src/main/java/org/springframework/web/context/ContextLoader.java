...
public class ContextLoader {
	...
	static {
		...
		try {
//			org/springframework/web/context/ContextLoader.properties    框架包的类的文件
	...
	protected WebApplicationContext createWebApplicationContext(ServletContext sc) {
		Class<?> contextClass = determineContextClass(sc); // org.springframework.web.context.support.XmlWebApplicationContext
	...
	protected void configureAndRefreshWebApplicationContext(ConfigurableWebApplicationContext wac, ServletContext sc) {
		...
		wac.setServletContext(sc); //wac == org.springframework.web.context.support.XmlWebApplicationContext
//		<context-param>
//	        <param-name>contextConfigLocation</param-name>
//	        <param-value>/WEB-INF/classes/applicationConfigLocation.xml</param-value>
//	    </context-param>
		String configLocationParam = sc.getInitParameter(CONFIG_LOCATION_PARAM);  // 
		if (configLocationParam != null) {
			// 占位符的定义不能在   ServletContext 和ServletConfig 中，因为在后面才做的实际指向
			wac.setConfigLocation(configLocationParam); // 设置配置文件的路径 org.springframework.context.support.AbstractRefreshableConfigApplicationContext
		}

		// The wac environment's #initPropertySources will be called in any case when the context
		// is refreshed; do it eagerly here to ensure servlet property sources are in place for
		// use in any post-processing or initialization that occurs below prior to #refresh
		ConfigurableEnvironment env = wac.getEnvironment(); // org.springframework.web.context.support.StandardServletEnvironment
		if (env instanceof ConfigurableWebEnvironment) {
			((ConfigurableWebEnvironment) env).initPropertySources(sc, null);// !!! 把数据源指向到 ServletContextPropertySource 和 org.springframework.web.context.support.ServletConfigPropertySource
		}

		customizeContext(sc, wac); // 对 XmlWebApplicationContext 应用初始化器
		wac.refresh();
	}
	...
	protected void customizeContext(ServletContext sc, ConfigurableWebApplicationContext wac) {
		...
		for (Class<ApplicationContextInitializer<ConfigurableApplicationContext>> initializerClass : initializerClasses) { // 配置文件有配置初始化类
		...
		for (ApplicationContextInitializer<ConfigurableApplicationContext> initializer : this.contextInitializers) {
			initializer.initialize(wac); // 应用初始化器
		}
	}
