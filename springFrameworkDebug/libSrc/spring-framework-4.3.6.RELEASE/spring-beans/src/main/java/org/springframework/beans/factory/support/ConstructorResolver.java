...
class ConstructorResolver {
	...
	public BeanWrapper instantiateUsingFactoryMethod(
		...
		String factoryBeanName = mbd.getFactoryBeanName(); // 工厂名
		if (factoryBeanName != null) {
			if (factoryBeanName.equals(beanName)) {
				throw new BeanDefinitionStoreException(mbd.getResourceDescription(), beanName,
						"factory-bean reference points back to the same bean definition");
			}
			factoryBean = this.beanFactory.getBean(factoryBeanName); // 获取工厂对象
			if (factoryBean == null) {
				throw new BeanCreationException(mbd.getResourceDescription(), beanName,
		...
		if (factoryMethodToUse == null || argsToUse == null) {
			...
			Method[] rawCandidates = getCandidateMethods(factoryClass, mbd); // 工厂的方法列表
			List<Method> candidateSet = new ArrayList<Method>();
			for (Method candidate : rawCandidates) {
				if (Modifier.isStatic(candidate.getModifiers()) == isStatic && mbd.isFactoryMethod(candidate)) { // 是工厂方法
	...
	private int resolveConstructorArguments(String beanName, RootBeanDefinition mbd, BeanWrapper bw,
		...
		TypeConverter customConverter = this.beanFactory.getCustomTypeConverter(); // org.springframework.beans.factory.support.DefaultListableBeanFactory
