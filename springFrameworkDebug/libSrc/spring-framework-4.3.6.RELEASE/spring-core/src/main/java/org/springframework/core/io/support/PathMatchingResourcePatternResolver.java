...
public class PathMatchingResourcePatternResolver implements ResourcePatternResolver {
	...
	public Resource[] getResources(String locationPattern) throws IOException {
		...
		if (locationPattern.startsWith(CLASSPATH_ALL_URL_PREFIX)) {
			...
			// locationPattern = "/WEB-INF/applicationContext.xml"   
			// locationPattern = "/WEB-INF/application*.xml"
			int prefixEnd = locationPattern.indexOf(":") + 1;
			if (getPathMatcher().isPattern(locationPattern.substring(prefixEnd))) {  // 不是表达式 org.springframework.util.AntPathMatcher
				// a file pattern
				return findPathMatchingResources(locationPattern);
			}
			else { // 是文件名
	...
	protected Resource[] findPathMatchingResources(String locationPattern) throws IOException {
		...
		Resource[] rootDirResources = getResources(rootDirPath); // 会递归
		Set<Resource> result = new LinkedHashSet<Resource>(16);
		for (Resource rootDirResource : rootDirResources) { // 目录资源 org.springframework.web.context.support.ServletContextResource
			...
			if (rootDirURL.getProtocol().startsWith(ResourceUtils.URL_PROTOCOL_VFS)) {
				...
				result.addAll(doFindPathMatchingFileResources(rootDirResource, subPattern)); // 查找出符合条件的文件列表
			}
		}
		if (logger.isDebugEnabled()) {
