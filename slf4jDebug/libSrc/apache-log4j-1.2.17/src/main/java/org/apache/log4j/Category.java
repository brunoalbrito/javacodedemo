...
public class Category implements AppenderAttachable {
  ...
  */
  ...
  public
  void callAppenders(LoggingEvent event) { // 调用Appender
	  int writes = 0;
	  ...
	  if(writes == 0) {
		  repository.emitNoAppenderWarning(this); // org.apache.log4j.Hierarchy
	  }
  }
  ...
  */
  ...
  void log(Priority priority, Object message, Throwable t) {
    if(repository.isDisabled(priority.level)) { // 日志级别不够 org.apache.log4j.Hierarchy
      return;
    }
    if(priority.isGreaterOrEqual(this.getEffectiveLevel()))
