...
new PropertyConfigurator().doConfigure(inputStream,
  ...
  */
  ...
  void configureRootCategory(Properties props, LoggerRepository hierarchy) {
    ...
    // org.apache.log4j.Hierarchy
      Logger root = hierarchy.getRootLogger(); // org.apache.log4j.spi.RootLogger
      synchronized(root) {
  ...
  */
  ...
  void parseCatsAndRenderers(Properties props, LoggerRepository hierarchy) {
	  ...
	  while(enumeration.hasMoreElements()) {
		  ...
		  if(key.startsWith(CATEGORY_PREFIX) || key.startsWith(LOGGER_PREFIX)) {
			  ...
			  Logger logger = hierarchy.getLogger(loggerName, loggerFactory); // org.apache.log4j.Hierarchy
			  synchronized(logger) {
  ...
  Appender parseAppender(Properties props, String appenderName) {
	  ...
	  String prefix = APPENDER_PREFIX + appenderName; // log4j.appender.appenderName1
	  String layoutPrefix = prefix + ".layout"; // log4j.appender.appenderName1.layout
	  ...
	  if(appender instanceof OptionHandler) { // 可选
		  if(appender.requiresLayout()) {
			  Layout layout = (Layout) OptionConverter.instantiateByKey(props,
					  ...
					  null);
			  if(layout != null) {
				  appender.setLayout(layout);// layout对象
		  ...
		  final String errorHandlerPrefix = prefix + ".errorhandler"; // log4j.appender.appenderName1.errorhandler=""
		  ...
		  if (errorHandlerClass != null) {
			  ErrorHandler eh = (ErrorHandler) OptionConverter.instantiateByKey(props,
					  ...
					  null);
			  if (eh != null) {
				  appender.setErrorHandler(eh); // 设置错误处理器对象
				  LogLog.debug("Parsing errorhandler options for \"" + appenderName +"\".");
				  parseErrorHandler(eh, errorHandlerPrefix, props, repository);
				  final Properties edited = new Properties();
				  final String[] keys = new String[] { 
						  errorHandlerPrefix + "." + ROOT_REF, // log4j.appender.appenderName1.errorhandler.root-ref
				  ...
				  for(Iterator iter = props.entrySet().iterator();iter.hasNext();) {
					  ...
					  if (i == keys.length) { 
						  edited.put(entry.getKey(), entry.getValue()); // 都没命中
					  }
				  }
		  ...
		  PropertySetter.setProperties(appender, props, prefix + "."); // 设置属性 log4j.appender.appenderName1
  ...
  void parseAppenderFilters(Properties props, String appenderName, Appender appender) {
    ...
    final String filterPrefix = APPENDER_PREFIX + appenderName + ".filter."; // log4j.appender.appenderName1.filter.
    int fIdx = filterPrefix.length();
    Hashtable filters = new Hashtable();
    Enumeration e = props.keys();
    String name = "";
    while (e.hasMoreElements()) {
      String key = (String) e.nextElement(); // log4j.appender.appenderName1.filter.property1
      if (key.startsWith(filterPrefix)) {
        int dotIdx = key.indexOf('.', fIdx);
        String filterKey = key;
        if (dotIdx != -1) {
          filterKey = key.substring(0, dotIdx); // log4j.appender.appenderName1.filter
          name = key.substring(dotIdx+1); // property1
        }
