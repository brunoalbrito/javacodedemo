...
public class SocketAsyncLongOutputAdapter {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				while (true) {
					try {
						if (canRun) {
							int interval = NumberUtils.toInt((String) configParams.get("HEARTBEAT_INTERVAL"), 30)
									* 1000;// 心跳间隔，单位：毫秒
							checkConnect(interval);
							Thread.sleep(interval);
						} else {
							int interval = NumberUtils.toInt((String) configParams.get("RUN_CHECK_INTERVAL"), 3) * 60
									* 1000;// 运行检测间隔，单位：分
							Thread.sleep(interval);
						}
					} catch (Exception e) {
	...
	private synchronized void connect() {
		...
		for (int i = 0; i < retryCount; i++) {
			try {
				...
				logger.info("对端[{}-{}:{}]连接成功，本地端口[{}]",
						new Object[] { hostName, hostAddress, hostPort, socket.getLocalPort() });
			...
			try {
				logger.info("对端[{}-{}:{}]连接失败，[{}]秒后重连",
						new Object[] { hostName, hostAddress, hostPort, retryInterval });
	...
	private void sendMessage() {
		...
		try {
			...
			logger.info("本地 ===>> 对端[{}-{}:{}] ## {}",
					new Object[] { hostName, hostAddress, hostPort, new String(bytes, charset) });
	...
	private void receiveMessage() {
		...
		try {
			...
			if (xmlLength <= 0 || xmlLength > maxSingleLength * 1024) {
				logger.error("连接[{} --> {}-{}:{}]出现账数据，自动断链：{}",
						new Object[] { socketKey, hostName, hostAddress, hostPort, headMsg });
				this.close();
				return;
			}
			/**
